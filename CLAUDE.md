# CLAUDE.md — AMUSE / Claude Code

## Role

Claude Code is AMUSE's primary implementation agent.

ChatGPT Work/controller owns project-level architectural review, task sequencing, YAGNI enforcement, and consequential scope decisions. Claude owns repository execution of approved work.
...
- Unity EditMode/integration tests;
- NDMF build tests;
...
- inspect Console output for unexpected errors;
...
CoplayDev MCP must not become a dependency of:
...
When implementation depends on version-specific Unity, NDMF, shader, or ecosystem behavior, verify the relevant current/pinned source instead of guessing.
