...
All test runs use the public `the project Assets folder` Unity instance through Unity MCP `run_tests` (EditMode). Before the first run, use read-only MCP discovery to confirm the connected instance's project root is `the project Assets folder`. If no public Unity Editor is running, **do not** substitute the private avatar testbed — report the blocked validation and stop.
