...
- Spec coverage: design sections 1 and 7 map to Task 1 steps 1 to 2 (schema, contract tests) and Task 2 steps 1 to 2 (gate swap, mirror swap). Design section 5 (keyword independence) is case 5. Design section 8 (the narrowing) is case 5's RED obligation. Design section 9 cases 1 to 9 are the plan's ten tests in `PoiyomiRimLightingAlphaTests`: design case 6 (absent property) was substituted one-for-one by the non-finite case through the same gate branch, and design case 8 (the mask-replace material keeps its exactly-one proof with a rim slot enabled and `_RimApplyAlpha` zero) is the `ReplaceMaskProof_WithRimEnabled_KeepsItsExactlyOneProof` guard. Design section 10 (honest expectation) needs no task: it is a corpus judgment, not code.
