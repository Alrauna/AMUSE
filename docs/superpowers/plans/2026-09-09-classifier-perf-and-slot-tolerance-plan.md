...
Pin `AMUSE-Census-Lab@13495ff7`.
Re-run the 100-triangle benchmark on `Roses` submesh 2.
Observe the speedup (expected: reduction from ~4.4 ms per triangle to <0.05 ms per triangle).
Confirm that `Roses Dress` optimizes when `IgnoreOutOfRangeMaterialSlots = true`.
Pin active instance back to Dev (`AMUSE@d5617927`).
