...
1. **Scope.** Production changes cover only the files in the File map. No Unity
   asset, `.meta`, scene, prefab, material, mesh, texture, import setting, animation
   asset, package manifest, or lockfile gets created or modified — with one explicit
...
3. **RED/GREEN.** Every behavioral step lands as: failing test observed for the named
   ...
   implemented in an earlier step of the same task (e.g. the Task 2 gate matrix
   against the scaffold, the Task 3 tuple/clone assertions) — has no observed RED by
   ...
   that verification in the checkpoint report for the step. Unverified immediately-green
   no one may cite these assertions as falsifiers.
4. **No placeholders.** Any scaffold used to get a behavioral RED (the Task 1 step 1
   deliberate opaque-profile mis-wiring, the Task 3 step 1 refuse-all eligibility
...
7. **Unity MCP gate.** Every Unity MCP operation starts with: enumerate instances
...
temporary mis-wiring that exists only so the Step 2 positive assertion fails
...
`_MainTex_ScrollRotate`. Otherwise the same texture-backed value shape the Poiyomi interpretation
produces (`PoiyomiMaterialSemantics.cs:740-755`): texture sample of
...
stricter than the shader rule `round` [B2 §10]), the `_Cutoff <= 0.9999` gate
...
2. Multiplier (coverage 5): `_Color.a = 0.8` variant → uniform
   ...
   `_Color.a` NaN / +inf / −inf → Unknown via the finite check of the interpretation —
   never the uniform-transparent `< 1` fallthrough (B2 §10 names
   non-finite as unsupported-multiplier refusal, and the resolver fallthrough would wrongly
...
3. Cutoff boundary at the classification layer (coverage 6, 7): `_Cutoff = 0.9999`
   ...
   non-finite `_Cutoff` → Unknown. Falsifies: the Poiyomi `<= 1` rule reused at either
...
7. Texture-evidence boundaries at the resolver seam (coverage 8, 9, 10, 15): one
   ...
   transparent base-level texels **outside** the mip-0 footprint of the tested triangle
   such that downsampling makes the covering higher-mip texel non-opaque, and
   assert the mip-0 support of the triangle is opaque before the higher mip flips the
   verdict to `MustRemainTransparent`, through the capture of the cutout frontend
...
8. Feature-variation invariance (coverage 14): through the verified-material seam,
   ...
   depends on runtime gates, never the define set (the executable form of the callback-100 argument). Falsifies: feature-set-dependent verdicts.
...
Implement the texture-backed arm mirroring the Poiyomi construction: read `_Color.a`
...
the existing vocabulary. Every immediately-green gate-matrix assertion records its mutation verification.
...
step. The wrapper gathers the opaque source evidence for the target and requires the full
...
4. Clone recipe read-back AND shader swap (coverage 17): the cutout fixture
   material of the shader with scrambled property values, `targetShader` = the
   **distinct** opaque fixture shader reference (`opaqueFixtureMaterial.shader`.
   the two schema-complete stand-ins of Task 2 Step 1
   ...
   reference equals `targetShader` and **differs from the source reference** (the swap is
   asserted, not preservation). 18 scalars, queue, tag read back canonical.
   the complete property set of the source material is unchanged (coverage 18, unit
...
the assertion records its mutation verification per the Step 2 run note.
...
This exists only so the Step 2 tests compile while failing behaviorally. Existing
...
recognized binding by family via `ResolveProofRelevant` against the own request of that family. `ConvertAdmittedMaterial`: `case LilToon: opaque = live; return None;`
...
with the property names of the family bucket, then seam or production sequence
...
`case Poiyomi:` byte-equivalent to the body today with the Poiyomi bucket. Update the
...
exact analog of the documented Poiyomi-seam substitution). Extend the
`AlphaSeparationPreparationTests` test-local request selector (`:854-856`) to mirror
the new production shape for `LilToonCutout`.

Run: the Task Step 4 end-to-end test green. Focused suites of Steps 2–4 green.
...
4. Cutoff slot outcomes through the full path (coverage 6): `_Cutoff = 0.9999`
   ...
   asserted separately in the Task 3 unit matrix.
...
design changes. Any design-level failure stops for controller review per the stop conditions of the spec).
...
mutation-verified per the Step 1 protocol and the verification is recorded.
...
2. The Task 5 end-to-end scenarios (Step 1 items 1 and 2 — full-artifact build and
   material-swap curve rewrite) **are** the public-fixture smoke path of the repository. Re-run it standalone and record the observed artifact
...
6. Report per the expected-report checklist of the controller. No commit, no push.
