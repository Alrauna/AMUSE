...
- **Testbed policy.** Every test in this plan creates and deletes temporary assets under `Assets/`. Run them **only** in the public development project. Confirm `Application.dataPath == "the project Assets folder"` before any test run. If only the private avatar testbed is reachable, **stop and report** rather than running there.
