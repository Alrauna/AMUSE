...
**Goal:** Add a pure, immutable, Editor-only planner. It partitions normalized triangle groups into opaque-candidate or retained-transparent membership. It preserves host-resolved source material-binding provenance.

**Architecture:** Host adapters are outside this milestone. They will normalize Unity, Blender, or other source topology/material layouts into immutable triangle-only inputs. The core planner copies these inputs and validates universal triangle structure. It scans outcomes once. It records source-local triangle ordinals and the caller-supplied material-binding index. Every returned plan is valid. Malformed normalized input throws. No generic unsupported status exists.
...
- Work on `feat/separation-plan`. Before editing, recheck the branch, status, merge base, and unrelated user changes.
...
- Preserve source vertex indices, triangle winding, repeated triangle occurrence identity, and source order. Use zero-based per-submesh triangle ordinals.
- Receive `SourceMaterialBindingIndex` explicitly and require a non-negative value. Preserve it exactly. Allow duplicate binding indices across submeshes.
- Never infer a material binding from the submesh index. Do not validate it against a host material count.
...
- Keep production types internal in the existing Editor analysis namespace. Reuse the existing `InternalsVisibleTo` boundary.
...
- Do not add Unity/Blender adapters, adapter interfaces, dependency injection, registries, serialization, public API, transformer abstractions, profitability policy, or shader logic. Do not add cache, jobs/Burst/GPU code, or NDMF integration.
- Preserve Unity `.meta` file pairing for the two future C# assets. Inspect GUID/scope changes before completion.
...
Keep all planner production types in one file and all planner tests in one file. This is the smallest coherent layout. Host adapters remain deferred. Do not create speculative files or interfaces for them.

## Shared production interfaces

Keep these names and signatures consistent across tasks. Change them only if an approved failing test demonstrates a concrete correction:
...
Plan output constructors/factories remain private. Thus, callers cannot construct contradictory cached facts. Copy a caller list with this pattern. Do not retain or return its array:
...
Keep expected dispositions, bindings, and ordinal lists literal in assertions. No test helper can reproduce planner membership logic.
...
**Interfaces:** Produces every shared type/signature and defensive input copies. Provides universal malformed-input validation and immutable empty plan factories. Provides a compiling planner that handles only the zero-submesh no-op.
...
Expected: branch `feat/separation-plan` and merge base `7c9c20b`. The merge base can differ if `main` advanced through a separately verified merge. Expect only the approved design/plan documentation changes. Stop if unrelated work appears.
...
Wait for Unity compilation. Read error-level Console entries. Expected: compile errors name the absent separation-plan types. Record this red state. Do not modify classifier or asmdef files to resolve it.
...
Set `TriangleCount` to `Indices.Count / 3` after copying. `MeshSeparationInput` validates non-negative `VertexCount` and non-null submeshes/elements. It copies the list and validates every copied vertex index:
...
Create private plan constructors/factories that copy result lists. `MeshSeparationPlanner.Create(null)` throws. For zero submeshes, return a plan with the exact facts that `EmptyMeshIsAValidNoOp` asserts. For non-empty valid inputs, return a compiling shell with no submesh records. Thus, Task 2 still has an executable red.

- [ ] **Step 6: Run Task 1 focused tests green**

Use Unity MCP `run_tests` in EditMode with these test names. Poll `get_test_job(wait_timeout: 60, include_failed_tests: true)`:
...
Run `git diff --check`, `git diff --stat`, and `git status --short`. Expected new implementation scope includes only the two C# files and their `.meta` pairs. This scope is in addition to the approved docs.
...
Expected: the production boundary compiles. However, assertions fail because the Task 1 shell emits no per-submesh record.

- [ ] **Step 3: Implement only unchanged per-submesh planning**

For each source submesh in ascending input order, create transparent ordinals `0..TriangleCount-1`, an empty opaque list, and `Unchanged`. Set `SourceSubmeshIndex` from the input list ordinal. Copy `SourceMaterialBindingIndex` from the corresponding input. Aggregate transparent counts. Do not add the `ProvenOpaque` branch in this task.

- [ ] **Step 4: Run unchanged tests green**

Expected: all four tests pass. `Unknown` and `MustRemainTransparent` remain retained. The empty record is present. Binding `7` is preserved without inference.
...
**Interfaces:** Adds the sole opaque-membership branch. Derives `WhollyOpaqueCandidate` versus `Split` from membership counts.
...
Expected: all-opaque and mixed membership assertions fail. This occurs because the unchanged-only implementation retains `ProvenOpaque`.
...
Expected: unchanged, all-opaque, mixed, and `Unknown` cases pass. The wholly opaque case does not represent an empty transparent output.
...
**Interfaces:** Confirms independent submesh planning and exact caller-supplied binding provenance. This includes reordered and duplicate binding indices.
...
This test proves that source submesh 0 can bind to 4. Source submesh 1 can bind to 2. The duplicate binding 4 remains legal and unchanged.

- [ ] **Step 2: Run the binding test and verify its result**

Expected: it fails if production inferred `SourceMaterialBindingIndex` from `SourceSubmeshIndex`, rejected duplicate bindings, sorted bindings, or omitted provenance. It can already pass because Task 2 copied the field generically. If so, record that the test characterizes the approved host-neutral contract.
...
Expected: plan records stay in source-submesh order. Empty groups remain represented. Bindings `4,2,4` and `8,3,8` are preserved exactly.
...
**Interfaces:** No new API. Proves that repeated index triples remain distinct occurrences. Per-submesh arrays eliminate global boundary arithmetic.
...
Expected: all three tests pass if ordinals are local, source append order is stable, and topology is copied unchanged. Preserve any observed red. Correct only the demonstrated provenance defect.
...
**Interfaces:** No new API. Uses adversarial tests to prove conservative membership, no profitability threshold, and caller-mutation protection.
...
Expected: all tests pass without production changes. This occurs if the explicit `ProvenOpaque` branch, source-order append, binding copy, and defensive collections are correct. Verify that each specified change would fail at least one named test. These changes include changing the opaque equality branch or sorting an ordinal/binding sequence. They also include retaining caller storage or inferring binding from the submesh index. They include omitting an empty submesh or adding a triangle threshold.
...
- Do not modify approved design/plan semantics unless implementation reveals a real contract conflict. If it does, return to the user.
...
Expected: zero failures. This proves that planner work did not change the upstream semantic boundary or fixture infrastructure.

- [ ] **Step 3: Run the complete EditMode suite**

Use Unity MCP `run_tests(mode: EditMode, include_failed_tests: true)` and poll the job. Record the observed total, passed, failed, skipped, and duration. Expected: zero failures and no unexpected warnings/errors.

- [ ] **Step 4: Read the final public Unity baseline**

Read `mcpforunity://instances` first. Select only the instance with project root `<repo-root>`. Then read `project/info`, `editor/state`, EditMode test discovery, and Console errors/warnings. Do not inspect a private testbed beyond identity discovery.

Expected: Unity `2022.3.22f1` and package `0.0.1`. Planner and classifier tests are discovered. The editor is not compiling. There are zero compiler errors and zero unexpected Console warnings/errors.
...
Confirm that only the approved spec/plan, planner production/test files, and their `.meta` pairs changed. Confirm that classifier code/tests, fixture JSON, asmdefs, package manifests/locks, dependencies, workflows, scenes, and private content are unchanged. Inspect each added `.meta` file. Confirm a unique stable GUID and correct asset pairing.

For each intended file that is still untracked, run `git diff --no-index --check -- NUL <path>` on Windows. Exit `1` is expected because the new file differs from `NUL`. Whitespace-error diagnostics are not expected.
...
Stage only reviewed files if the user has authorized commits. Use coherent messages such as:
...
This plan is documentation only. Stop here until the user approves both `docs/superpowers/specs/2026-08-15-separation-plan-design.md` and this implementation plan. After approval, use inline `superpowers:executing-plans` by default. Also use `superpowers:test-driven-development`, `ponytail:ponytail`, and `superpowers:verification-before-completion`. Do not dispatch subagents, commit, push, or open a PR without separate authorization.
