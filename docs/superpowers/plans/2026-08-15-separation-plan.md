...
Read `mcpforunity://instances` first, select only the instance whose project root is `the project Assets folder`, then read `project/info`, `editor/state`, EditMode test discovery, and Console errors/warnings. Do not select or inspect a private testbed beyond identity discovery.
