...
**Goal:** Establish AMUSE's smallest authoritative NDMF PlatformFinish foundation: exact fail-closed lifecycle capability, eager immutable host extraction for the current alpha proof path, one bounded prepare/apply operation, an explicit first-mutation failure boundary, and NDMF-owned generated assets without implementing alpha mutation or DAO cooperation.

**Architecture:** Export one AMUSE NDMF plugin whose sole pass runs in `BuildPhase.PlatformFinish`; phase authority, rather than Meshia/AAO identity edges, places every normal `Optimizing` producer before the semantic barrier. The pass captures lifecycle facts, performs a dry analysis through eagerly captured immutable renderer values and per-material requested evidence, and retains `BuildContext` only inside the bounded operation so later concrete mutation can prepare through `AssetSaver` and cross one explicit apply boundary. Lifecycle/semantic refusal and explicit unsupported preparation decisions preserve input; unexpected preparation or apply exceptions propagate to NDMF as build-blocking internal failures.

**Tech Stack:** Unity 2022.3.22f1, C# Editor-only code, NDMF 1.14.4 public plugin/`BuildContext`/`IAssetSaver` APIs, Unity Package Manager `PackageInfo`, NUnit EditMode tests, existing `Alrauna.Amuse.Editor` and `Alrauna.Amuse.Tests.Editor` assemblies. No new dependency, assembly, runtime component, SDK assembly reference, reflection dependency, or persistent asset system.
...
- NDMF 1.14.4 exposes no public token that separates SDK preprocessing from manual non-Play processing. The supported observable bucket is therefore named `NonPlayNdmfBuild`; it authorizes only this bounded NDMF operation and does not claim that an upload was attempted. Exact SDK `3.10.4` remains required for the normal-host build-abort contract.
- Positive mutation is unavailable during Apply-on-Play, for missing/unknown versions, for a non-VRChat platform, or when `AssetSaver`, its current container, `ObjectRegistry`, or `ErrorReport` is unavailable.
...
- Add no edge to NDMF's `Generate portable components` or `CheckMipStreamingPass`. Their characterized behavior is irrelevant to the current alpha theorem.
...
- The reusable material layer contains no shader-family semantics, universal shader IR, arbitrary property DSL, reflection schema, provider interface, or plugin registry. A second concrete request exercises reuse; it does not create a second production consumer.
...
- Only `AmusePreparationDecision.Refused(reason)` represents expected unsupported preparation. An exception from preparation or apply is unexpected and must escape the operation so NDMF records a build-blocking internal failure.
...
Expected green: exact tuple permits; every mismatch refuses with its exact reason.
...
- Task 2 sets only execution/lifecycle; renderer counts stay zero until Task 5. Lifecycle refusal lives only in `HostLifecycleCapability.Refusal`; candidate refusal continues to use `RendererAnalysisRefusal`, so the diagnostic domains cannot be confused.
...
Expected green: the deliberately late-named Optimizing producer is before AMUSE, the after-AMUSE observer sees both states, and no producer identity edge exists.
...
- Produces the unchanged constructor/provider signatures, but storage becomes `Dictionary<TextureSourceId, AlphaTextureData>`; no `Texture2D` is retained.
...
- Unsupported, destroyed-at-capture, unreadable, mipmapped, malformed, non-`Texture2D`, or unidentifiable inputs are omitted and later refuse as before.
...
Assert `PoiyomiMaterialSemantics.AlphaEvidenceRequest` does not request `_Cutoff`, `_Mode`, `_SrcBlend`, `_DstBlend`, `_EmissionColor`, `_EmissionStrength`, `_BumpScale`, or `_BumpMapUV`. Assert `LilToonMaterialSemantics.AlphaEvidenceRequest` does not request Poiyomi-only `_AlphaForceOpaque`, `_MainIgnoreTexAlpha`, or `_AlphaToCoverage`. Focused alpha/attestation tests are the positive guard that every property each current consumer does read is requested: any omission throws rather than returning Unknown. The complete semantic suite proves the untouched non-alpha equations retain their behavior.

- [ ] **Step 2: Write failing per-material request, texture deduplication, and immutability tests**

Import one readable asymmetric RGBA32 texture. Assign it to a Poiyomi fixture's `_MainTex` and a lilToon fixture's `_EmissionMap`. Give the two inputs different scalar and texture-property requests while requesting the same texture fact set.
...
Then mutate pixels, importer sampling/color interpretation, both material assignments, and scale/offset; destroy the live materials and delete the texture asset. Assert every captured primitive and alpha byte remains unchanged. Walk all reachable captured fields and fail on `UnityEngine.Object`. This single test proves request isolation and operation-wide stable-source deduplication together.
...
Validate a null input list, a null request, blank property names, undefined flags, duplicate names within one category, and the same property requested under incompatible scalar/color/vector/texture categories. A null/destroyed source material is an ordinary unsupported input and produces empty evidence for its request. Presence-only overlap with a typed request is allowed. `Combine` performs deterministic ordinal set union and bitwise-unions repeated texture-property flags for callers that genuinely have multiple consumers of the same material.

Capture in two private passes inside the one static method:

1. For each input independently, resolve only that input's requested names with `Shader.FindPropertyIndex`; read `HasProperty` once per distinct requested name, validate its `ShaderPropertyType` without enumerating the shader, and copy only its requested scalar/color/vector/texture assignments and scale/offset into transient builders. Scalar accepts Float, Range, or Int. Across all inputs, resolve stable source identities and union requested texture flags by `TextureSourceId`.
2. Capture each identified texture's operation-wide requested union once through `UnityTextureEvidence` and Task 3, capture each unidentified assigned texture independently, construct immutable shared `CapturedTextureEvidence`, then construct one final material evidence result per input in original order. Never merge property-name requests across different inputs.
...
In each existing frontend, build one static alpha request from the exact property constants and gate/schema arrays read by its alpha equation and source-attestation material checks. Request presence-only facts for schema checks; scalar/color/vector facts only for matching alpha/attestation reads; and texture flags only for `_MainTex` facts the alpha path consumes. Request `AlphaChannel` only for Poiyomi `_MainTex`, the only current sampled alpha-classifier source. Do not request active color space or base-color/emission/normal-only fields.

`UnityMaterialSemantics.CaptureAlphaMaterials` examines only each live material's shader name to select Poiyomi's request, lilToon's request, or a private empty request for an unsupported/null material. It creates one aligned `MaterialEvidenceCaptureInput` per material and calls host capture once, so family requests remain isolated while all inputs share texture deduplication. It then gathers only the selected source-evidence family and returns immutable `CapturedAlphaMaterial` values in input order. Do not run the lilToon filesystem/include scan or request lilToon fields for a Poiyomi material, and vice versa. Move material-dependent attestation reads needed by this path to `CapturedMaterialEvidence`; retain file/package/source reads in purpose-specific code. Defensively copy `LilToonSourceEvidence.CompiledFeatures`; nested canonicalization records already copy their inputs.

Add a semantic-layer batch test with one material per family. Assert the Poiyomi evidence rejects access to lilToon-only `_Invisible`, the lilToon evidence rejects access to Poiyomi-only `_AlphaForceOpaque`, and both families still produce their prior alpha results. Do not add a family registry or selector interface; two explicit shader-name branches match the existing dispatcher style.
...
Expected green: each family request satisfies only its consumer reads, `_Cutoff` stays absent until the synthetic request asks for it, different per-material requests coexist in one batch, shared texture evidence is reference-identical across that batch, and all semantics/diagnostics remain unchanged.
...
Move live reads through renderer/topology/material validation, positions, UV0, indices, and captured materials into `Capture`. Defensively copy every array/list. Create the mutation target only after acceptance. Refusal returns null snapshot/target and performs no semantic work.
...
Move resolution/classification/planner composition to `Analyze(UnityRendererAlphaSnapshot)`. Build alpha lookup from each captured material's `Evidence.Textures` entries whose stable source identity and requested alpha channel are present. This overload must not touch `Renderer`, `Mesh`, `Material`, `Texture`, `AssetDatabase`, or `QualitySettings`.
...
Expected green: facade behavior remains, later live mutations cannot change captured proof, and anonymous Optimizing output is analyzed without an identity edge.
...
Validate arguments. Refused lifecycle returns before prepare. Invoke prepare with no catch. Return `PreparationRefused` only when the returned decision is explicitly refused; return for no-mutation. Invoke apply with no catch, then return `Mutated`.
...
3. Current PlatformFinish actors are treated as irrelevant; no tail introspection exists.
...
6. Each Poiyomi/lilToon material receives only its applicable alpha/attestation evidence request, even when both occur in one capture operation; unrelated family properties remain absent, while multiple concrete requests may be combined only for the same material.
7. Texture-channel storage is shared by stable captured source identity within a material-capture operation, and texture pixels, requested material/source facts, renderer relationships, positions, UV0, topology, and indices are eager immutable values.
...
12. Unexpected preparation exceptions propagate before apply, and apply exceptions propagate after the first mutation; both are build-blocking through NDMF and no rollback is promised.
...
The next branch may consume these immutable analyses and the operation boundary to prepare/apply an alpha separation plan. It must add exact opaque-target proof/construction and concrete mesh/material mutation; this foundation pre-authorizes neither.
...
- Unity test CI hosting; current release/listing workflows remain unchanged.
