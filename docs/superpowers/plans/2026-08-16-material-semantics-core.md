...
Use only the public AMUSE Unity project. Discover `mcpforunity://instances`, select the instance whose root is `the project Assets folder`, wait for compilation, then run:
...
Use the verified public `the project Assets folder` Unity instance and run all EditMode tests. Record total, passed, failed, skipped, and duration. Expected: zero failures and no unexpected Console errors/warnings. This includes unchanged classifier, exact geometry, fixture integrity, separation planner, smoke tests, and the new semantic tests.
