...
| `RenderType` override tag | `Opaque` — set explicitly | `[DECISION]`: the vendor leaves stale override tags in place for non-Multi; the converted clone must read back `Opaque` |
