...
> triangle-level alpha separation, which require separate semantic or representation proofs,
> which are already-opaque/target-only states, which are internal helpers, and which should be
> explicitly refused because opaque separation cannot preserve their behavior?

The product requirement: every official lilToon material-shader family receives an
evidence-backed applicability disposition. Positive optimization is not required for every
family; families that cannot preserve intended behavior must receive an explicit, justified
refusal or a clearly scoped future investigation.

## 2. Bounded non-implementation scope

This note closes the inventory and establishes the investigation roadmap. It does not:
...
The existing B1 scope (regular, non-Lite, non-Tessellation, non-Multi, no-outline cutout) is
unchanged and remains the only positively scoped first slice.

Labels: `[SOURCE]` pinned-package fact (commit-pinned paths in §3); `[MEASURED]` prior
executed observation from the merged B1 note (`2026-08-30-liltoon-opaque-characterization.md`);
`[INFERENCE]` bounded conclusion; `[DECISION NEEDED]` unresolved product/architecture choice.

## 3. Source pin and method

- Upstream pin: lilToon tag `2.3.4`, commit
  `252fd8cfc46106d4967e95b3f2c788418502f227` (`Assets/lilToon/package.json:2` declares
  `"version": "2.3.4"`). All `[SOURCE]` paths below are relative to `Assets/lilToon/` at that
  commit. The pin was cloned read-only into a temporary directory outside AMUSE; it is not a
  Unity project and nothing was installed into AMUSE.
- Merged repository base: `main` at `0e90f01` (merge of PR #38, B1 characterization), branch
  `investigate/liltoon-family-applicability`. The controlling investigation
  (`2026-08-30-liltoon-opaque-conversion.md`) and B1 (`2026-08-30-liltoon-opaque-characterization.md`)
  were read in full and treated as claims to verify; no contradiction with the pinned source was
  found. The unmerged `design/liltoon-cutout-alpha-semantics` branch (cutout alpha-semantics
  design) is visible repository reality but is not merged context; F0's roadmap references it
  only as in-flight work.
- Method: closed-asset enumeration of every `*.shader` file in the pinned package
  (`Shader/` — no `.shader` asset exists anywhere else in the package), mechanical extraction of
  each asset's internal `Shader "…"` name, reconciliation against every field of
  `Editor/lilShaderManager.cs` and the vendor mode map in `Editor/lilMaterialUtils.cs`
  (`SetupMaterialWithRenderingMode`, `:18-330`), per-container pass/`UsePass`/`GrabPass` extraction,
  and targeted reads of pass includes for alpha/refraction/fur/gem/lite behavior.
- No new Unity measurements were made; runtime-behavior claims reuse B1 `[MEASURED]` rows only.
...
- **13 internal shaders** — 9 hidden pass assets consumed via `UsePass`/includes, 4 editor
  bake/support shaders. Not avatar-material families.
...
- `Editor/lilShaderManager.cs:8-79` declares 60 fields: 52 material fields ↔ 52 material assets
  (1:1 by internal name, verified mechanically), 6 pass fields, 1 baker field, and
...
- 65 = 52 + 13. Every discovered `Shader` asset is classified; no manager field is unaccounted.
- Officially generated identities: `.lilcontainer` assets are compiled to Shader assets by the
  scripted importer (`Editor/lilShaderContainerImporter.cs:23-25`,
  `Editor/lilMaterialUtils.cs:717-730` accepts them as lilToon). These are user-authored
  integration identities, not official package assets; they are out of the official inventory and
  are recorded as out of F0 scope (`[DECISION NEEDED]` for any future support policy).
...
- **R** — Explicit refusal candidate: evidence indicates the transformation would discard or
  alter defining behavior.
- **I** — Internal/helper shader: evidence dependency or editor/build tool.
- **U** — Unknown / decision needed.

Every disposition below states: source evidence, transformation opportunity, preservation risk,
the exact missing proof, and the next action. Per falsifiability requirements:

- groupings are stated with counterexamples;
- alpha-one sufficiency is addressed per mode class (§7.0);
- every positive/conditional candidate has a target-preservation check;
- every refusal has a reconsideration condition;
- no disposition was upgraded because evidence was missing; increasing uncertainty never
...
1. **Mode is a shader-asset identity for non-Multi.** `SetupMaterialWithRenderingMode`
   ...
   (`material.shader = …`), never a property/keyword toggle. The vendor writes blend/depth state
   (`:63-71,96-104,171-179,192-194,213-219,232-238,242-248,261-263`), restores the pre-call queue
   for non-Multi (`:266`), and resets the common tail (`:268-329`, Gem gets `Cull=0`/`ZWrite=0`;
   `_ZTest=4` is skipped only for `transparentMode == TwoPass`, `:277-280`).
2. **Mode map coverage:** Opaque/Cutout/Transparent select lite/tess/outline variants;
   **Refraction selects only `ltsref`/`ltsmref`; RefractionBlur only `ltsrefb`; Fur/FurCutout
   select only `ltsfur`/`ltsfurc`/`ltsmfur`; FurTwoPass only `ltsfurtwo`; Gem only
   `ltsgem`/`ltsmgem`** (`:181-264`) — the specialized modes have no outline/lite/tessellation
   variants reachable via the vendor conversion.
3. **Multi mode is `_TransparentMode`** (0..6 enum, `ltsmulti.shader:561`), read only by editor
   tooling (`lilMaterialUtils.cs:24-37,353+`); no HLSL include reads it (`[SOURCE]` — grep of
   `Shader/Includes/` and `ltsmulti.shader` HLSL finds only the property declaration). Effective
   rendering mode is carried by **keywords** derived at edit time
...
4. **Callback 100 after NDMF:** `External/Editor/VRChatModule.cs:22` (`callbackOrder 100`),
   `:60-61`/`:85-86` run `SetShaderSettingBeforeBuild(materials, clips)` and
   `SetupMultiMaterial(materials, clips)`. The clips variant (`lilMaterialUtils.cs:476-495`)
   scans animation curve bindings and **enables keywords on all Multi materials** for three
   features (`_RimDirStrength`→`GEOM_TYPE_LEAF`, `_MainTexHSVG`/`_MainGradationStrength`→
...
5. **Outline is a set of full passes** (`FORWARD_OUTLINE`, `FORWARD_ADD_OUTLINE`,
   ...
   Offset, BlendOp, Blend, AlphaToMask) and the vendor tail resets `_OutlineZTest=2` (Less)
...
7. **Refraction samples the background:** `GrabPass {"_lilBackgroundTexture"}`
   ...
   and a FORWARD_BLUR pass (`lts_ref_blur.shader:738-787`). Gem is additive:
   `Blend One One, Zero One`, `ZWrite=0`, `Cull=0`, GrabPass, FORWARD_PRE pass
   (`lts_gem.shader:578-592,628,745-790`).
8. **Fur generates shells:** the fur pass extrudes vertices by `_FurVector` (optionally blended
   ...
   (`Shader/Includes/lil_common_vert_fur.hlsl:169-195,477-502`; `lts_fur.shader:619`).
   Fur blends `SrcAlpha/OneMinusSrcAlpha` with `_FurZWrite=0`; FurCutout
   is LIL_RENDER 1 with `One/Zero` + `_FurZWrite=1`; FurTwoPass adds `_PRE` shell passes
   (`lilMaterialUtils.cs:213-248`; `lts_furonly_two.shader` `UsePass` list).
...
- Evidence: LIL_RENDER 0 pass asset forces alpha to one and compiles out the alpha path
...
- Role in mixed animation-reachable sets: an attested `lilToon` material is `AlreadyOpaque` and
  maps to itself with no clone (existing preparation machinery, controlling investigation §3 item 4).
  In a slot mixing a cutout source with an opaque sibling, the opaque sibling participates in the
  admitted-set completeness check without conversion. **Target-only**: it is the conversion target
  of the first slice.
- Missing proof: none (B1 measured its identity/digests). It is never a separation source.
...
- Evidence: B1 measured the exact installed source/target identities, digests, pass
  relationships (`Hidden/ltspass_cutout` with `LIL_RENDER 1`), clone/assignment behavior, queue
  and recipe read-backs (`[MEASURED]`, B1 §§5-9). The cutout equation is
...
- Preservation risk: alpha-mask/dither/dissolve generated paths and `_AlphaToMask` coverage (§7.0).
...
- Next action: B2 characterization/semantic design (already commissioned; in-flight on an
  unmerged design branch).

### 7.3 Regular cutout + outline (`Hidden/lilToonCutoutOutline`) — **C: Conditional**

- Evidence: vendor opaque target is `Hidden/lilToonOutline`, not `lilToon`
  ...
  outline pass (controlling investigation §10) — not a preservation move.
...
- Preservation risks: (a) target attestation does not exist (`Hidden/lilToonOutline` + its
  ...
  extrapolated outline surfaces should remain continuous **if** outline properties are cloned
  identically — but this must be proven, including per-pass draw order across submeshes and
...
- Evidence: LIL_RENDER 2, blend `One/OneMinusSrcAlpha`, queue 2460, `_ZWrite` default 1
  (`lts_trans.shader:579-592,673`); four-pass container (FORWARD/FORWARD_ADD/SHADOW_CASTER/META).
  At `a≡1`, `One/OneMinusSrcAlpha` degenerates exactly to `One/Zero` (§7.0), so a proven-opaque
  triangle's own output is blend-equivalent to opaque.
...
- Vendor-target check: vendor maps `ltst → lts` for Opaque (`lilMaterialUtils.cs:130-132`) — the
  official target exists and preserves lighting passes. Alpha-one blend degeneration is
  arithmetic, but the **proof** (texture/animation evidence over the LIL_RENDER 2 alpha path) is
  strictly larger than B2.
- Exact missing proof: transparent alpha semantics investigation (a B2 successor), plus the
...
- Evidence: container `UsePass`es only FORWARD/SHADOW_CASTER/META — **no FORWARD_ADD**
...
- Counterexample to grouping with Normal: the official opaque target `lilToon` **has**
  ...
  deliberately lacks — a visual change under multi-light conditions, not preserved by any
  official no-additive opaque asset (none exists in the inventory).
...
- Next action: fold into the transparent investigation as an explicit target-identity sub-question.

### 7.6 Regular transparent TwoPass (`Hidden/lilToonTwoPassTransparent`) — **C: Conditional, likely refusal-leaning**

- Evidence: adds the FORWARD_BACK back-face pre-pass (`§6.6`); vendor skips `_ZTest=4` reset for
  TwoPass (`lilMaterialUtils.cs:277-280`), i.e. the vendor itself treats TwoPass `_ZTest` as
  non-canonical; queue 2460.
- Preservation risk: proven triangles leaving the source submesh also leave FORWARD_BACK
  ...
  a source whose visibility may depend on a different comparison (controlling investigation §6
  already refuses this).
- Exact missing proof: a pass-level characterization showing moved-triangle behavior under
  FORWARD_BACK removal is invisible; absent that, formal refusal.
...
- `lts_oo` (opaque outline-only): renders only `ltspass_opaque` FORWARD_OUTLINE +
  FORWARD_ADD_OUTLINE at Geometry queue (LIL_RENDER 0). Already-opaque rendering; not an
...
- Evidence: Lite shares the alpha equation shape in its own pipeline (§6.9): opaque/cutout/
  transparent modes with the same `LIL_RENDER` semantics, reduced feature set, separate pass
  assets (`ltspass_lite_*`) that are **not** manager fields and carry their own digests.
- Counterexample to "Lite ≡ regular": different pass assets and reduced feature set mean AMUSE's
  existing `lilToon` attestation does not cover any Lite identity; the equations being alike does
  not make the attestations interchangeable.
...
- Disposition per mode: Lite opaque **A** (target mapping under a future Lite attestation);
  ...
  **C** (inherits §7.4-7.6 questions in the Lite pipeline); Lite outline **C** deferred behind 7.3.
- Next action: single bounded "Lite pressure" investigation after the regular cutout slice exists.
...
- Analysis: base-triangle classification transfers to subdivided fragments if displacement is
  ...
  and properties; the UV domain of a subdivided fragment is contained in its base triangle's UV
  domain, so a base-domain alpha proof covers subdivided samples). The correct target is a
  tessellation opaque shader (`ltstess`), preserving tessellation on the appended submesh.
- Counterexample to "tess ≡ regular + tess": the appended-submesh machinery must reference the
  same vertices while the tess shader displaces per-vertex — proof needed that submesh splitting
  does not change hull adjacency/winding behavior at the split boundary.
- Exact missing proofs: tess source+target attestation, displacement-equality argument,
...
- Evidence: LIL_RENDER 2 + GrabPass + queue 2900 (§6.7). The fragment output is
  ...
  This is a real narrow state, not a guess. However: (a) proving internal alpha ≡ 1 requires the
  ...
  and `GrabPass` semantics (what the sampled background contains, ordering against other 2900+
  materials and other avatars' GrabPass materials) are
  representation-relevant even when individual triangles are opaque; (c) RefractionBlur adds a
  second GrabPass and a blur pass whose output exists only to feed refraction sampling.
- Is the narrow state meaningful? Unknown without characterization — avatar refraction is used
  precisely to show distorted background (alpha < 1 in practice). `[INFERENCE]` the population of
  alpha≡1 refraction triangles is likely small, but this is not evidence.
- Exact missing proofs: LIL_RENDER 2 alpha-equivalence semantics; queue/GrabPass ordering
  characterization; RefractionBlur blur-pass invariance. Absent all three, refusal.
- `[DECISION NEEDED]` whether the controller ever wants this investigation commissioned;
  F0's recommendation is formal refusal unless a real-avatar need appears.
...
- Evidence: appearance is generated shell geometry — `_FurLayerNum` (1–3) geometry-shader-emitted
  shells extruded along `_FurVector` with gravity (§6.8). Fur mode blends `SrcAlpha/OneMinusSrcAlpha`
  with `_FurZWrite=0` at queue 3000; FurCutout is LIL_RENDER 1 but the shells still define the
  look; FurTwoPass adds `_PRE` shell passes.
- Why alpha separation is inapplicable: moving base triangles to an appended submesh **removes
...
- Counterexample to "FurCutout is cutout": identical LIL_RENDER 1 alpha machinery, yet refused —
  render mode alone does not determine disposition.
- Reconsideration condition: a future representation proof capable of reasoning about the
  generated shell geometry **as rendered** (e.g., separating whole-fur renderers as a unit) —
  a separate, explicitly scoped investigation; not a byproduct of the cutout work. There is no
...
- Evidence: one shader asset carries all modes; effective mode is keyword-carried, derived at
  edit time from `_TransparentMode` (§6.3); callback 100 additionally enables three
...
- Consequences: (a) an NDMF-time observation of a Multi material's keywords and blend state is
  ...
  (`§6.3`) may disable shadow/depth passes — pass-enable state is material state AMUSE must
  capture; (c) Multi Fur/Refraction/Gem inherit their families' refusals (§7.11, §7.12, Gem §7.14).
...
- Evidence: additive `Blend One One, Zero One`, `ZWrite=0`, `Cull=0`, GrabPass, FORWARD_PRE pass
  (§6.7). The gem look is additive accumulation of background/sparkle over whatever is behind;
  no alpha state degenerates `One/One` to `One/Zero` while preserving the effect (the added
  contribution IS the appearance).
- Reconsideration condition: none realistic under the current AMUSE concept; would require a
  different transformation class (e.g., proving an additive contribution is zero and dropping the
  material — a different feature).
...
- `ltsover` (regular overlay): `UsePass`es `Hidden/ltspass_transparent` FORWARD + FORWARD_ADD at
  queue 2460 (`lts_overlay.shader:673-676`) — Normal-transparent-class; inherits §7.4's alpha
  semantics and gate work. Missing proofs: §7.4's theorem plus an overlay-purpose target
  decision (`[DECISION NEEDED]` §13.4).
- `ltsoover` (regular overlay OnePass): FORWARD only, **no FORWARD_ADD**
  (`lts_overlay_one.shader:673-674`) — OnePass-class; inherits §7.5's additive-light target
...
- `ltslover`, `ltsloover` (Lite overlays): consume `Hidden/ltspass_lite_transparent` FORWARD
  (+FORWARD_ADD for `ltslover`; FORWARD only for `ltsloover`) — Lite attestation surface per
  §7.9 **and** the corresponding Normal/OnePass class questions.
- All four: intended behavior is overlay layering over the base avatar surface (typically on
...
**U (0)** — no family is left without a disposition; two `[DECISION NEEDED]` markers live
inside C/R rows (OnePass target §7.5, refraction commissioning §7.11), plus the scope
confirmations in §13.
...
The specialized assets are assigned only when the *target* mode is specialized (`:181-264`).
The vendor's opaque target therefore exists but is not preservation-safe for any specialized
source; AMUSE cannot borrow it, and no official specialized-preserving opaque target exists.
Any future positive support would require AMUSE-generated targets, which the current policy does
...
- Callback 100 (after NDMF; `VRChatModule.cs:22,60-61`) regenerates shader files per project
  ...
  - **Cutout/transparent proofs** must be invariant under `LIL_FEATURE_*` regeneration — the
  ...
  - **Already-opaque mapping** is callback-safe: identity/queue/tag of attested assets are
...
- Attestation pins one pass identity (`Hidden/ltspass_opaque`) and `OpaqueRenderMode 0`;
  `InterpretVerifiedAlpha`'s constant-1 theorem is valid only under LIL_RENDER 0.
...
Derived from the matrix; bounded tasks, no mega-task. "Gates a vertical increment" = production
work cannot start for that family before the row's deliverable exists.
...
Ordering rationale: alpha-proof machinery must exist before it is reused (1 → 4/6/6b/8); outline
needs target attestation work unrelated to transparent ordering (3 independent of 4); Lite
transparent (6b) must follow regular transparent (4/5) because it inherits their target and pass
questions; Multi lifecycle is orthogonal to alpha equations (7 after 2 only because Multi cutout
would reuse its recipe); refusals (9) have no upstream dependency and can land any time.
...
2. `[DECISION NEEDED]` Refraction narrow-state investigation: commission or formalize refusal
   (§7.11, roadmap row 10).
3. `[DECISION NEEDED]` `.lilcontainer`-generated shader identities: confirm they are out of
...
targets for specialized modes; the Multi callback keyword facts; the
  outline/two-pass/refraction/fur/gem structural facts
that shape future proofs; a dependency-ordered roadmap with bounded tasks.
...
**Yes.** The inventory is closed and mechanically reconciled (52 + 13 = 65, every manager field
and every asset accounted); every material-entry family carries an evidence-backed disposition
with explicit missing proofs or refusal bases; the roadmap is dependency-ordered and bounded; no
production design, digest pinning, or implementation was performed. F0 does not gate on the
`[DECISION NEEDED]` items, which are future-scope choices, not inventory or roadmap gaps.
...
to it: state explicitly whether its theorem template is recorded so roadmap rows 4, 6, 6b, and 8 can
reuse the shape without re-deriving it. No other row should start first: every other positive
