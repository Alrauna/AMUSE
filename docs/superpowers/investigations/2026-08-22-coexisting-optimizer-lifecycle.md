...
This document records an architectural investigation. It does not implement an NDMF plugin, AMUSE alpha mutation, DAO cooperation, a plugin adapter, a callback registry, or a generalized interoperability framework.
...
- Unity probes: no new probe was required; exact source distinguished the lifecycle and structural-opportunity hypotheses
...
State-based composition remains the default interoperability model. Modular Avatar, VRCFury, AAO, and completed prior transformations normally leave ordinary Unity state for AMUSE to analyze without tool-specific provenance. DAO is the one demonstrated v1 case where a narrow bilateral contract can retain materially more safe optimization than ordinary state and incidental callback order provide.

The decision hierarchy is:

1. **Safest minimum:** Candidate A — authoritative AMUSE-before-DAO order plus protected-target no-rewrite behavior.
2. **Recommended v1:** restricted Candidate B — the same authoritative order plus a fixed, versioned opaque-target preservation profile, with Candidate A as the mandatory fallback.
3. **Longer term:** Candidate C fused planning/execution only if future profiling demonstrates material costs that Candidates A and B cannot recover.

Candidate B is the **highest-performing realistically supportable v1 contract in the architectural and optimization-opportunity sense**. Measured VRChat runtime performance has not yet been benchmarked.
...
This investigation does not reopen later lilToon callback-100 observation, generated-shader observation, a late AMUSE SDK commit gate, or cross-callback build-attempt identity for the targeted alpha transformation.

The strongest discovered broad semantic-barrier candidate is the beginning of NDMF `PlatformFinish`, because every NDMF `Optimizing` plugin necessarily completes before it. It is not the selected production mutation phase. NDMF describes `PlatformFinish` as platform-specific cleanup and validation territory and `Optimizing` as optimization territory. A subsequent design must choose between deliberate `PlatformFinish` mutation and an authoritative final-`Optimizing` ordering strategy.
...
This investigation deliberately does not choose whether that guarantee is implemented through a DAO NDMF participant, a DAO hook, an AMUSE bridge, SDK callback restructuring, or another narrow mechanism.

## Question

Can AMUSE establish a sufficiently authoritative effective-avatar state before proof and a sufficiently bounded remainder after mutation in realistic projects containing DAO, AAO, VRCFury, Modular Avatar, NDMF, lilToon, and unknown add-ons?

More specifically:

- which preceding operations can be treated as ordinary state producers;
- which later operations must be ordered, modeled, proven irrelevant, or refused;
...
- whether fused DAO-AMUSE execution is justified for v1;
...
- callback 100 is represented by a narrow exact-version alpha/coverage projection;
...
- no later AMUSE SDK authorization is required for that targeted path;
...
- **Empirical evidence:** behavior observed through a controlled exact-version probe. No new empirical evidence was created in this investigation.
...
The current AMUSE repository was inspected from Unity renderer extraction through immutable mesh-separation planning. It currently analyzes and plans. It has no NDMF plugin, opaque-target creator, mesh/material mutation executor, or complete animation/material-swap reachability extractor.
...
The exact pinned pipeline is reconstructed as follows.

### 1. NDMF early SDK hook at `-11000`

NDMF creates its `BuildContext` and executes its early phases through `Transforming`.

Modular Avatar performs its principal Resolving and Transforming work inside this interval. It generates or rewrites hierarchy, armature, meshes and bones, controllers and clips, menus and parameters, material setters and swaps, reactive components, and related ordinary Unity state.
...
- without Modular Avatar: `-1025`, equal to NDMF's optimize hook;
...
Tool provenance is needed only when:

- required semantics are not represented in the resulting state;
...
- explicit ordering is required;
- a versioned cooperation contract supplies proof authority unavailable from ordinary state.

An unknown producer before the barrier is not automatically unsafe. If it has completed and AMUSE completely supports the resulting state, its history is irrelevant. An unknown proof-relevant writer after the barrier is unsafe for the affected candidates unless it is ordered, modeled, proven irrelevant, or causes refusal.
...
The key questions are whether AMUSE can see the effective result and whether it needs to know who produced it. For MA, VRCFury, and AAO, current evidence answers yes and no respectively, subject to AMUSE supporting the resulting semantics. DAO's cooperating future write is the demonstrated exception.
...
No MA-specific AMUSE contract is justified by current evidence.

MA does affect DAO's current callback selection, but that incidental relationship is not promoted into AMUSE's ordering contract.

## VRCFury coexistence

VRCFury's main proof-relevant generation completes before NDMF Optimizing. Its resulting controllers, clips, material-property bindings, swaps, hierarchy, and generated assets are ordinary Unity state.
...
No VRCFury-specific AMUSE contract is justified by current evidence.

The late parameter-compressor conclusion is deliberately narrow: it is irrelevant only to the current targeted alpha theorem at the pinned revision and only because it does not alter the covered mesh/material/clip/material-property bindings. Future AMUSE semantics involving expression parameter behavior must characterize it separately.

## AAO coexistence

AAO performs the majority of its mutation in NDMF `Optimizing`, including mesh edits, skinned-mesh and material-slot merging, object/component sweeps, material/property/texture cleanup, animation mappings, and animator graph optimization.

AAO exposes the qualified plugin identity `com.anatawa12.avatar-optimizer`. NDMF's standard plugin-ordering mechanism can target it when an explicit edge is needed. Its internal late type-name placement is implementation behavior, not AMUSE's preferred proof authority.

For v1 compatibility, AAO should finish before AMUSE. AMUSE can then consume AAO's resulting mesh, material, controller, and hierarchy state. This may miss an opportunity for AAO to optimize AMUSE-added slots, but it avoids proving AAO's later original-state-dependent transformations.

AAO's component-information and shader-information APIs are useful ecosystem precedent. Current evidence does not demonstrate an information deficit requiring an AAO-AMUSE contract.

No AAO-specific AMUSE contract is justified by current evidence.

## DAO transformation and write-domain map

DAO's `Optimize()` workflow can perform the following operations, depending on settings and compatibility:
...
DAO output is not intrinsically dependent on transformation history. Final shader, mesh, material, controller, and texture state encode the runtime result in principle. The current AMUSE implementation cannot interpret arbitrary DAO-generated shader and MaterialID semantics, so DAO-first Full output causes safe opportunity loss rather than a requirement for a universal provenance record.

## DAO ordering comparison

### DAO before AMUSE

AMUSE would receive DAO's completed renderer, mesh, material, shader, texture-array, and controller output.

DAO Basic output may remain understandable as ordinary state under the scoped conditions below. DAO Full can generate custom shaders, MaterialID transport, texture arrays, and rewritten animation outside AMUSE's current semantic model. Current AMUSE must refuse affected alpha candidates.

This direction preserves DAO consolidation but loses AMUSE alpha opportunities. It also places AMUSE after DAO's SDK callback, outside the natural active NDMF mutation interval unless another host arrangement is introduced.

DAO-before-AMUSE is therefore a compatibility fallback, not the preferred primary coexistence lifecycle.

### AMUSE before DAO

DAO receives AMUSE's separated geometry, preserved transparent material role, and proof-sensitive opaque target.

Basic-compatible structural consolidation can remain useful. Full semantic rewriting can invalidate the theorem AMUSE used unless DAO supplies an exact preservation guarantee or avoids rewriting the protected target.
...
DAO documents deliberate Basic-preset tradeoffs. They must be classified against the current targeted theorem rather than described as globally harmless.
...
- unsupported or uncertain protected-target operations are skipped.

Candidate A is the mandatory safe fallback.

DAO's existing shader comment `//d4rkAO:incompatible_shader` is useful precedent. Exact DAO documentation and source show that it disables:
...
All other optimizations remain eligible under DAO's own checks. The existing marker is shader-wide, however, and therefore too coarse for a shared vendor lilToon shader or a single protected material. Candidate A records required behavior, not that exact implementation surface.
...
DAO may perform only explicitly admitted transformations whose preservation of that profile can be established for the exact supported DAO, shader, and profile combination. If preservation cannot be established, DAO automatically uses Candidate A for the affected protected target.
...
4. no new unmodeled vertex, primitive, or fragment suppression path is introduced;
...
These postconditions protect AMUSE's authorization theorem. They do not claim that every independent DAO tradeoff is erased.
...
The current source does not itself certify the profile. DAO's transformer parses and re-emits shader source, replaces constants, changes structures and function signatures, transports mesh/material IDs, creates arrays, injects visibility checks, and may alter pass or variant code. Candidate B therefore requires explicit bilateral ownership and exact-version evidence before production use.
...
Candidate C is rejected for v1. It remains only a possible longer-term direction if profiling demonstrates meaningful duplicated build work or runtime costs that Candidates A and B cannot recover.
...
Candidate A and restricted Candidate B lose three renderer updates and three draw slots relative to unrestricted Shader Toggles on this fixture. Candidate B deliberately does not recover that opportunity in v1 because visibility translation is a new coverage theorem.
...
AMUSE's opaque split can retain a separate opaque draw role, but the opaque triangles can gain depth writes, early-Z/occlusion behavior, and avoid transparent sorting and overdraw behavior. Neither the fixture counts nor these known mechanisms establish a measured net VRChat runtime result.

No aggregate performance score is invented. Real runtime effects depend on triangle distribution, screen coverage, material complexity, platform, driver, visibility, animation, and the surrounding avatar.
...
- its proof-relevant work has completed;
...
A later unknown actor is classified by domain where reliable evidence permits:
...
- absence of a reliable write-domain contract cannot be replaced by callback-name inference.
...
No generic callback registry is justified.
...
No exhaustive Cartesian version matrix is claimed.
...
DAO must be able to leave a protected target semantically unrewritten while retaining independently safe structural work where its existing checks and the scoped Basic conditions permit it.

### Candidate B requirement

DAO and AMUSE may recognize a fixed opaque-target preservation profile. DAO may apply an explicitly certified operation only when it guarantees the profile's postconditions for the exact DAO/shader/profile combination. This investigation does not define the marker format, API, callback, data representation, or implementation ownership split.
...
Each case uses Candidate A when the ordering and no-rewrite guarantee remain available; otherwise the affected AMUSE candidate is refused.
...
- DAO cooperating work must begin only after authorized AMUSE mutation completes;
...
- arbitrary generated shaders outside AMUSE's semantic model;
...
No probe was run because exact DAO source directly distinguished the options:
...
A fixture would have demonstrated those branches but could not certify Candidate B's semantic postconditions. That certification belongs with the future exact contract and executable target path.

No disposable Unity assets or project state were created.
...
No production tests were required or run because production code did not change. No Unity probe, Census Lab, private avatar, upload, external project mutation, or runtime benchmark was used. DAO, AAO, VRCFury, and Modular Avatar were not modified.
