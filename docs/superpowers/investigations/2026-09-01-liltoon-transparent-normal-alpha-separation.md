...
This note is roadmap row 4 of the merged family inventory
(`2026-08-30-liltoon-family-applicability.md` §11). That note classified the identity
**C — conditional** (§7.4). It named the missing proof: "transparent alpha semantics
...
variant, OutlineOnly, Lite, Tessellation, Multi, Overlay. Also refused: Fur (all
variants), Gem, FakeShadow, Refraction, RefractionBlur, `.lilcontainer` identities,
non-identity `_MainTex_ST`. Also refused: UV channels other than 0 and
trilinear/anisotropic/mirrored/asymmetric sampling. Also refused: any generalized
render-state IR, adapter framework, planner, or third-party API. **This note designed
and implemented no production behavior.**

**Verdict: GO.** A controller-authorized scratch measurement discharged both
prerequisites this note originally opened (P1, P2) on the same day. See §3.4, §9.4,
§12, §17.
...
makes. `[DECISION NEEDED]` a choice the controller must make.

The `[PREREQUISITE]` label appeared in the first draft of this note for two facts that
were then unavailable. Both are now `[CLOSED — MEASURED]` (§12).

**Census Lab and private avatar data were not used, not inspected, and not modified.**
This investigation enumerated two existing Unity instances read-only and never targeted
them. It issued no Unity MCP call against either. All Unity work ran in a throwaway
project outside AMUSE (§3.4), now deleted.

## 2. Repository and base state

| Fact | Value | Evidence |
|---|---|---|
| Repository | the AMUSE working copy, remote `https://github.com/Alrauna/AMUSE.git` | `[MEASURED]` `git remote -v` |
...
The investigation inspected the two package files and left them **untouched**. It did
not stage, commit, push, rebase, stash, or publish anything.
...
directory outside AMUSE. That directory is not a Unity project. The process installed
nothing into AMUSE and touched no package manifest.
...
- All `[SOURCE]` paths below are relative to `Assets/lilToon/` at that commit. All line
  numbers are for the **LF-normalized** file. The tree ships CRLF, and every file cited
  here is byte-identical with and without CRLF stripping except for line terminators.
  See §3.2. There the raw and LF SHA-256 values coincide for every cited file. The files
  are already LF in the tree.
...
**Cross-check that anchors these digests to the merged AMUSE attestation.** The LF
SHA-256 of `Shader/lts.shader` (`5206bec2…a9c704`) is **exactly** the merged
...
The LF SHA-256 of `Shader/lts_cutout.shader` (`c83d73a2…836178`) is exactly the merged
`CutoutShaderCanonicalDigest` (`:359-360`). `[MEASURED]`

Two independent material-shader assets reproduce their installed-measured pins from the
tag tree. Thus the AMUSE canonicalization is a **no-op for material-entry shader
assets**. Those assets contain no generator-varying region and no include line that
canonicalization rewrites. `[INFERENCE]`

Consequence: `ea247d3cd6ecb09ad4aeefdcad37480c0dffa40d594a3b457624097f2372ba13` was a
strongly-supported candidate for the transparent material-shader canonical digest. The
scratch probe (§3.4) later **confirmed it as the measured installed value**. This method
could not derive the *pass* asset digest. The raw LF hash of `ltspass_opaque.shader`
(`fe32b23b…`) differs from its merged pin `6b6c30c1…` `[MEASURED]`, because
canonicalization removes the generator-varying regions. The probe therefore measured it
directly (§3.4).

### 3.3 Method

1. Closed enumeration: the transparent container and its pass asset. It captured
...
5. Exhaustive grep of every `LIL_RENDER` conditional in `Shader/Includes/*.hlsl`, then
   a direct read of each `LIL_RENDER == 2` site.
6. Reconciliation against the vendor mode map (`Editor/lilMaterialUtils.cs`).
7. The investigation reconstructed the current AMUSE implementation and the merged
   design record from the checked-out tree at `a3c547b`.
8. One web search, for the **Unity** built-in `_DitherMaskLOD` texture (§9.4). This note
   explicitly treats its result as **not** sufficient primary evidence. §3.4 replaced it
...
The probe ran on 2026-09-01 under explicit controller authorization. The first draft of
this note recorded P1 and P2 as open before that.

| Fact | Value |
|---|---|
| Editor | Unity `2022.3.22f1` (`887be4894c44`), macOS, Metal, Apple M2, Gamma color space, batch mode |
| Project | a throwaway `-createProject` project in a temporary directory, **outside** AMUSE; `Application.dataPath` asserted in every probe's own output |
...
digests that AMUSE already pinned, from this independent install:
...
three things at once. The copied algorithm is faithful. This install matches the install
that produced the merged pins. The generated pass output is reproducible across installs
and across two weeks. `[MEASURED]`

**Regeneration is real, and confined.** The probe compared the imported package against
the pristine extracted zip. The material-entry shaders (`lts.shader`, `lts_trans.shader`,
`lts_cutout.shader`) are byte-identical to the upstream tag after LF normalization.
**The import step rewrites every `ltspass_*.shader`.** The transparent pass diff versus
upstream is exactly three `#pragma skip_variants` lines plus one
`#define LIL_FEATURE_VRCLIGHTVOLUMES_WITHOUTPACKAGE`. That is per-project variant
stripping, driven by which packages are installed (`Editor/CurrentRP.txt` = `BRP` /
`Metal`).

The canonicalizer of AMUSE reports exactly `removedRegions = 2` on every `ltspass_*`
asset and `0` on every material-entry asset. That is why the pass digest is stable across
installs despite the rewrite. This concretely vindicates the refusal of the merged design
to hash pass assets whole. `[MEASURED]` `[INFERENCE]`
...
The material digest equals the LF hash of the upstream tag because material-entry assets
undergo no regeneration. The pass digest does **not** equal any upstream hash. This
happens because the import rewrite and the later canonicalization change the pass asset
(§3.4).
...
The pass asset declares **eight** passes. In order: `FORWARD_BACK` (`:795`), `FORWARD`
(`:844`), `FORWARD_OUTLINE` (`:892`), `FORWARD_ADD` (`:945`), `FORWARD_ADD_OUTLINE`
(`:993`), `SHADOW_CASTER` (`:1045`), `SHADOW_CASTER_OUTLINE` (`:1085`), `META`
(`:1126`). The no-outline Normal container references exactly four of them, in the order
above. `[SOURCE]`
...
All three declare identical tags and share the same pass asset. Thus the shader-asset
...
Only `FORWARD_BACK` (`ltspass_transparent.shader:831`) defines `LIL_TRANSPARENT_PRE`,
which activates the `_PreColor`/`_PreCutoff`/`_PreOutType` fragment arm
(`lil_pass_forward_normal.hlsl:405-410`). Normal never compiles that arm. `[SOURCE]`
...
**Every `FORWARD`, `FORWARD_ADD`, `SHADOW_CASTER`, and `META` declaration is
byte-identical between the two assets.** This covers the stencil block, the
`Cull`/`ZClip`/`ZWrite`/`ZTest`/`ColorMask`/`Offset`/`BlendOp`/`Blend`/`AlphaToMask`
expressions, and the `#pragma` sets. `[MEASURED]` `[SOURCE]`

The compiled feature sets are also identical. Both assets (and `ltspass_cutout.shader`)
declare the **same 103** `LIL_FEATURE_*`/`LIL_OPTIMIZE_*` symbols. A `diff` of the
sorted define lists returns empty. `[MEASURED]`
...
- Source and target participate in **exactly the same four light modes**. Conversion
  ...
  ForwardAdd — F0 §7.5.)
- Every render-state difference between source and target lives in **material property
  values**, never in pass declarations. The existing evidence model of AMUSE (captured
  scalars + effective queue + `RenderType`) is therefore sufficient to describe the whole
  render-state delta. It needs no pass model and no render IR.
- The single compile-time constant `LIL_RENDER` carries every *behavioral* difference.
  AMUSE already reads its value from the live pass
...
non-canonical is `_DstBlend` (10 → 0). Note this differs from cutout. Its one
...
2. IDMask — `#if defined(LIL_FEATURE_IDMASK) && !defined(LIL_NOT_SUPPORT_VERTEXID) && !defined(LIL_LITE)`
   (`:362`). A masked vertex collapses. `_IDMaskControlsDissolve` drives
...
The shader initializes `dissolveActive`/`dissolveInvert` to `true`/`false` (`:55-56`)
and packs them into the v2f (`:426`).
...
The cause is step 11, which never compiles. `[SOURCE]`
...
- Base pass at `a ≡ 1`: `rgb *= 1` — exact identity, matching the empty macro of the
  target.
- **ForwardAdd at `a ≡ 1`: `rgb *= saturate(_AlphaBoostFA)`.** This is **not** an
  ...
  (`lts_trans.shader:32`). The inspector cannot produce a value below 1. A serialized or
  animated value can go below 1. `saturate` of a value in `[0,1)` scales the additive
  light contribution down. The opaque target has **no** premultiply, so a material with
  `_AlphaBoostFA < 1` would get *brighter* additive lighting after conversion. `[SOURCE]`
  This is a load-bearing, transparent-only gate that has no cutout analogue.

### 5.4 The `LIL_RENDER == 2` RGB arms are all `× fd.col.a`

Exhaustive enumeration: every `LIL_RENDER == 2` site in `lil_common_frag.hlsl`
`[MEASURED]`:
...
Distance fade is the single exception. **Every other RGB divergence between
`LIL_RENDER 2` and `LIL_RENDER 0` is a multiplication by `fd.col.a`.** At `a ≡ 1` it
collapses to the expression of the target. `[SOURCE]` `[INFERENCE]`

Distance fade is different. Its two arms are *structurally* different, not merely
scaled. The transparent arm fades toward `fadeColor * _DistanceFadeColor.a` and also
writes alpha. The opaque arm fades toward `fadeColor`. `[SOURCE]`
...
Both arms multiply `distFade` by `_DistanceFade.z`. So `_DistanceFade.z == 0` forces
...
(`lil_pass_forward_normal.hlsl:242`, `:418`, `lil_common_frag.hlsl:1980`). **No pass
asset defines it, and `Editor/lilToonSetting.cs` does not define it either.** (The
103-symbol lists in §4.3 do not contain it.) The `_DepthFade*` uniforms are absent from
`lil_common_input_base.hlsl` / `lil_common_input_opt.hlsl` / `lil_common_input.hlsl`.
They survive only as commented-out lines inside the function body
...
`#if LIL_RENDER > 0` (`:12`, `:116`) wraps the whole file body. Thus the **shadow caster
of the opaque target performs no alpha work and never discards.** `[SOURCE]`

For `LIL_RENDER 2` the chain is: UDIM discard (`:8-10`) → main UV (`:21-22`) → main
color (`:31-32`). It continues: layers (`:41-54`) → alpha mask (`:58-61`) → dissolve
(`:65-81`) → fur (absent) → dither (`LIL_RENDER == 1` only, `:93-95`), then:
...
- This chain has no parallax step. So with `_UseParallax == 0`, the sampled domain
  matches the forward pass. `[SOURCE]`
...
**The coverage of the transparent source at `a ≡ 1` is not decidable from lilToon
source at this one place.** Measurement resolved it — see §9.4.
...
(`lil_common_frag.hlsl:1969-1975`). At `a ≡ 1` the meta output equals the output of the
target. `[SOURCE]` (META is lightmap-baking only and does not participate in VRChat
avatar rendering. This note covers it for completeness, not because it is load-bearing.)
...
path. §5.1 walks the coverage sites of the vertex function. §5.2 walks the fragment
linearly. §5.4 enumerates every `LIL_RENDER == 2` conditional found by exhaustive grep.
§5.6 walks the subpass chain, and §5.7 the meta chain. Outline-only sites
(`LIL_OUTLINE`) are not referenced by the supported container.
...
independent writer in rows 1–23 is proven off, dead, or an identity at `a ≡ 1`. The
proof comes *before* the claim. Rows 19–23 are the writers that do **not** exist in the
cutout frontend. A copy-paste of the cutout gate list would be unsound exactly there.
...
Measured against the merged eligibility gates of AMUSE
...
`IsZeroDestinationFactorAtAlphaOne` accepts `OneMinusSrcAlpha` by design
(`LilToonOpaqueConversion.cs:459-463`). At `a = 1` it evaluates to 0, so
...
`_SubpassCutoff`) remain. They separate the transparent source from the merged
eligibility function.

**Queue/order.** Moving proven triangles from 2460 to 2000 changes draw order. Both
2000 and 2460 sit below the 2501 transparent-sorting threshold of Unity. Both therefore
draw front-to-back opaque-sorted, and the source already declares `_ZWrite 1` (required
by the gate). The moved triangles are depth-determined, not order-determined.

`[INFERENCE]` This is the same representation change the merged cutout slice already
accepted (2450 → 2000). The Conservative policy explicitly permits it
...
**Stencil, `_Cull`, `_ZClip`.** These are `[_Property]`-driven in both pass assets.
`new Material(source)` copies them. The recipe does not write them, exactly as in the
...
Every gate in §6 is an ordinary `material.<Property>` float/color/vector and is
therefore animation-reachable. `_MainTex_ST` rides the `ScaleOffset` kind of the texture
request, whose derived binding name AMUSE already recognizes. The exact-singleton
admission machinery (`AdmittedMaterialStates`) expresses the complete requirement. A
non-singleton or disagreeing binding on any of them refuses the slot. A binding on a
name outside the request refuses the renderer batch. `[SOURCE]` (current tree)

Relative to the merged cutout request
(`LilToonCutoutMaterialSemantics.AlphaEvidenceRequest`,
`LilToonCutoutMaterialSemantics.cs:100-142`), the transparent request needs **three
additions**: `_AlphaBoostFA` (scalar), `_SubpassCutoff` (scalar), `_DistanceFade`
(vector). It could also drop `_UseDither` (inert, §6 row 16). Everything else is
identical.
...
not material properties. The existing `GatherSourceEvidence` pattern gathers them at the
barrier. The merged design already reads effective queue and `RenderType` live during
preparation. The material binding syntax of Unity cannot address either. `[SOURCE]`

### 8.3 Callback 100 (`External/Editor/VRChatModule.cs`)

The mechanism stays unchanged from B2 §9. At avatar build the module (callback order 100,
after NDMF) calls `lilToonSetting.SetShaderSettingBeforeBuild(materials, clips)`. That
call re-unpacks each in-use shader from its `.lilinternal` container with only the used
`LIL_FEATURE_*` defines. It rewrites `lil_common_input_opt.hlsl`. Then it restores the
committed files afterwards. `SetupMultiMaterial` touches Multi materials only.

Per-feature invariance for **this** family:

- `LIL_RENDER` comes fixed per pass asset (`ltspass_transparent.shader:672`), and the
  generator never rewrites it. `[SOURCE]`
- The core equation — main sample, `_Color` multiply, `clip(a - _Cutoff)`, premultiply,
  emission blend, fog — is unconditional code with no `LIL_FEATURE` gate. `LIL_RENDER`
  selects `LIL_PREMULTIPLY`, `OVERRIDE_BLEND_EMISSION`, and the fog macros, not a
...
not a prerequisite. Residual honesty stays as in B2. The uploaded per-avatar
shader artifact is not a stable attestable asset. So the design attests the committed
source plus the invariance argument. It does not digest the uploaded artifact.
`[INFERENCE]`

### 8.4 Pass enables

`new Material(source)` copies the serialized `Material.SetShaderPassEnabled` state, so
it transfers to the clone. This matches how the merged cutout slice treats it, and
introduces nothing new. `[INFERENCE]` `_AsOverlay`-driven pass enables are a Multi-only
mechanism (F0 §6.3) and are out of scope.
...
It renders with material `M` under the attested regular no-outline lilToon 2.3.4
Transparent **Normal** source. `T` is *proven opaque* iff all of the following hold.
The evaluation checks every clause against captured, admitted evidence. Unknown
...
1. **Identity.** `M.shader` is `Hidden/lilToonTransparent`, GUID
   ...
   `_lilToonVersion == 45`. It carries scanned `#define LIL_RENDER 2`, canonical
   material digest `ea247d3c…2ba13` and canonical pass digest `700a6076…52517f`
   *(both `[MEASURED]`, §3.4)*. The shared 37-file include-tree digest `6e2dce6c…fd46`
   and the canonicalization provenance the merged verifier already requires also apply.
2. **Cutout-shared gates, captured and finite, exactly zero.** `_Invisible`,
   ...
   `_DissolveParams`.
3. **Transparent-only gates.** `_Cutoff` finite and `<= 1` (§9.2). `_AlphaBoostFA`
   finite and `>= 1` (§5.3). `_DistanceFade` finite with `_DistanceFade.z == 0` (§5.4).
   And the §9.4 `SHADOW_CASTER` subpass condition.
4. **UV domain.** `_MainTex_ScrollRotate == (0,0,0,0)` per binary32 component and
   `_MainTex_ST == (1,1,0,0)` per binary32 component — the exact-identity boundary of
   the family (§9.3).
5. **Texture.** `_MainTex` is an assigned `Texture2D` with an admitted format, full mip
   residency, filter Point or Bilinear, `wrapU == wrapV ∈ {Clamp, Repeat}`. It has a
...
6. **Per-triangle classification.** For every mip level, the exact classifier over the
   UV hull of the triangle finds every intersecting texel alpha `== 255`. This includes
   the point/bilinear footprint neighbourhood and wrap normalization. So
...
7. **Animation closure.** Every property named in clauses 2–4 and 6 is in the captured
   request. Every binding reaching `M` on those properties is exact-singleton against
   the serialized default of the material. Any unrecognized proof-relevant binding
   refuses the renderer batch.

**Conclusion.** Under clauses 1–7, chain alpha is `a ≡ 1` at the clip on every fragment
of `T`. This covers `FORWARD`, `FORWARD_ADD`, `SHADOW_CASTER`, and `META`. Then:
...
- every `…ApplyTransparency`, emission-blend, and fog `× a` term equals the
  expression of the target (§5.4).
...
- the ForwardAdd `Max`/`One One` state is already the canonical state of the target (§4.4).
- the target renders the same geometry through byte-identical pass declarations
  (§4.3).

Therefore the `FORWARD`, `FORWARD_ADD`, and `META` output of the converted triangle is
**arithmetically equal** to the output of the source. `SHADOW_CASTER` is equal too. The
measurement at `a ≡ 1` gave exactly `1 - _SubpassCutoff` for the subpass clip argument
(§9.4). Under the clause-3 gate `_SubpassCutoff <= 1` the fragment survives, matching
the unconditional shadow coverage of the target. **Verdict: `ProvenOpaque` across
`FORWARD`, `FORWARD_ADD`, `SHADOW_CASTER`, and `META`.**
...
round-to-nearest. **Exact representability of `1 - c` is neither claimed nor needed.**
It does not hold across the whole declared range `c ∈ [-0.001, 1.001]`
(`lts_trans.shader:21`). Only the *sign* is load-bearing, and correctly-rounded
subtraction preserves it:
...
- For finite binary32 `c <= 1` the exact difference `1 - c` is **nonnegative**.
  ...
  `fl(1 - c) >= 0` and `clip` **keeps** the fragment. (Sterbenz also makes the
...
- For finite binary32 `c > 1` in the declared range the exact difference lies in
  `[-0.001, -2^-24]`. That is strictly negative, and orders of magnitude above the
  binary32 underflow threshold. No rounding can flush it to zero. `fl(1 - c) < 0` and
  `clip` **discards**.

So at `a ≡ 1` the fragment survives iff `c <= 1`. `[SOURCE]` `[INFERENCE]`

This is materially different from cutout. The cutout forward pass uses the *coverage*
...
refuse the exact band `c ∈ (0.9999, 1]` for no reason. `[DECISION]`

The design should carry a **family-specific** bound of `<= 1`. The falsifier suite must
contain a `_Cutoff = 1.0` positive case that fails an implementation which copies the
cutout constant. It must also contain a `_Cutoff = 1.001` negative case at the upper end
of the declared range. There the source discards the triangle entirely, and conversion
would materialize invisible geometry.
...
**family-blind** affine resolver. It explicitly did **not** authorize lilToon. Its
fragment applies `lilRotateUV(outuv, 0)` with no zero-angle early-out
(`lil_common_functions.hlsl:424-435`, re-verified at this pin). It evaluates
...
Therefore the transparent family inherits the identity-only boundary unchanged. It also
inherits the **C6 baseline clause** of the affine spec. The identity slice absorbs the
rotate-at-zero fragment noise as a declared abstraction, exactly like the shipped cutout
and opaque slices. No new modelling, no new risk, no parity break. `[SOURCE]`
`[DECISION]`
...
`clip(_DitherMaskLOD[uint3(x%4, y%4, 15)].a - _SubpassCutoff)`. The shadow caster of
the opaque target performs no clip at all (`frag_alpha:12,116`).
...
resource. Its contents are not in lilToon source. They are not in the shipped shader
source of Unity. They are not stated in the Unity documentation.

What is available:

- The built-in shader of Unity itself uses the identical sampling expression and clips
  at `0.01` (`UnityStandardParticleShadow.cginc`, "Our dither texture is 4x4x16":
  <https://github.com/TwoTailsGames/Unity-Built-in-Shaders/blob/master/CGIncludes/UnityStandardParticleShadow.cginc>).
  That proves the sampling convention, **not** the slice-15 values. The `0.01` threshold
  of Unity would pass values that the default `0.5` of lilToon would reject.
- A well-known secondary source (Catlike Coding, *Rendering 12*,
  ...
  as 16 patterns of 4×4. Pattern 0 is empty. The sequence ends with **all pixels
...
instead. The claim is load-bearing. If slice 15 contained a zero texel, the shadow of a
fully-opaque transparent material would render dithered. Conversion would *add* shadow
coverage.
...
The probe located it among the loaded `Texture3D` objects. A probe shader sampled it
with the same `tex3D(...).a` expression that lilToon uses:
...
proves it reads the intended channel. The monotone fill from slice 0 to slice 15 matches
the documented dither ladder. The `1 − 1 ulp` control proves the measurement is sensitive
at exactly the boundary the theorem depends on. That control is itself a warning: the
exact-`1` requirement in clause 1 is load-bearing, not cosmetic. One ulp below `1` lands
on slice 14, whose first texel is `0`.

**Scope of the measurement.** The `filterMode` of the installed texture is `Point`. The
exact `lilSampleDither(…, alpha = 1)` expression returned `1` at all 16 positions on
Metal / Gamma. That is the whole of the observation. This measurement makes no claim
that the result is independent of filter mode. Slice 14 is *not* all-ones. So a
filtering mode that blended neighbouring slices is not covered by this measurement.
...
zero-measurement fallbacks below are **not** shipped. They remain in the record only, as
what would have been sound without the measurement.
...
readback, the probe ran a rendered comparison. One side: a `Hidden/lilToonTransparent`
material at its declared defaults with a fully-opaque `_MainTex` and `_Color.a = 1`, on a
shadow-casting cube. A hard-shadow directional light lights it over a Standard receiver.
The other side: the output of `LilToonOpaqueConversion.PrepareCanonicalOpaqueClone` from
**AMUSE itself** for that same material.
...
The unmodified merged code path produced the clone. That path throws rather than returns
on a read-back disagreement. It accepted a *transparent* source and reported `lilToon`,
queue 2000, `RenderType = Opaque`, `_DstBlend 10 → 0`. That is direct evidence that the
target module is source-independent (§11, §13.4). The converted material is
pixel-identical in both the forward and the shadow-receiving result. The negative
control proves the comparison can detect a difference. `[MEASURED]`

### 9.5 Worked positive example

A fresh Transparent Normal material sits at its declared defaults —
`_Color = (1,1,1,1)`, `_Cutoff = 0.5`, `_SubpassCutoff = 0.5`, `_AlphaBoostFA = 10`,
`_DistanceFade = (0.1, 0.01, 0, 0)`, `_MainTex_ST = (1,1,0,0)`,
`_MainTex_ScrollRotate = (0,0,0,0)`. Every gate in clause 2 sits at 0, the `_MainTex` is
fully-opaque at every mip, and no animation binds. So: `a ≡ 1`, `clip(0.5)` keeps, both
premultiplies are identities, distance fade is the identity, and the blend degenerates.
Every colour pass matches the target exactly, and with slice 15 measured all-ones (§9.4)
the shadow pass matches too. The probe rendered this exact material against its real
canonical clone, and it came out bit-identical (§9.4). **`ProvenOpaque`** — `[SOURCE]`
derivation, `[MEASURED]` confirmation.
...
whole avatar. No row makes a transformation *more* aggressive.
...
Checked against the stop-condition list of the task:
...
`TransparentShaderCanonicalDigest = ea247d3cd6ecb09ad4aeefdcad37480c0dffa40d594a3b457624097f2372ba13`,
`TransparentPassCanonicalDigest = 700a607661f2cc43550452795d8eae0634509dbd07b4e8c381d9412fcc52517f`,
shared `IncludeTreeDigest = 6e2dce6c…8fd46`. Measured with the canonicalizer of AMUSE
itself in a scratch install that first reproduced all five existing pins. Identical
across two Editor sessions. See §3.4.

### P2 — `_DitherMaskLOD` slice 15 / `_SubpassCutoff` `[CLOSED — MEASURED]`

Slice 15 is uniformly alpha `1`, with slice-0, channel, and 1-ulp controls. A
bit-identical rendered shadow comparison against the real clone of AMUSE independently
confirmed it. Gate: finite `_SubpassCutoff <= 1`. See §9.4.
...
passDigest)`. `OpaqueProfile` (`:402-410`) and `CutoutProfile` (`:412-420`) are static
instances, and `Verify`/`Gather` take the profile as a parameter (`:970`, `:1183`,
`:1388`). A `TransparentProfile` is **six new constants plus one static instance plus
two thin wrappers** (`TryVerifyLilToonTransparentIdentity`,
`GatherTransparentSourceEvidence`). **`[INFERENCE]` No refactor, no interface, no
registry.** This directly answers what the task asked. Yes: the private profile
model accepts a transparent profile cleanly, and it was evidently designed to.
...
recipe, clone preparation and validation. (ii) The cutout source eligibility gate set.
(iii) The shared blend/depth/queue lemmas, result types, and Unity enum constants.
Responsibility (i) is proven source-independent (§11). Responsibilities (ii) and (iii)
are not target concerns at all.

One concrete symptom of the current conflation: `ConversionEvidenceRequest` carries
**19** properties — the 18 recipe properties **plus `_Cutoff`**, concatenated by
`BuildConversionSchema` (`LilToonOpaqueConversion.cs:199-214,703-712`). The recipe never
writes `_Cutoff`, and only the source clip gate reads it. It is source-eligibility
evidence living in the schema of the target. Both alpha requests already capture it
independently (`LilToonCutoutMaterialSemantics.cs:97-126` and the transparent sibling).
So it can move without losing capture or animation closure.
...
forbids. It would put the cutout `0.9999` bound, the transparent `<= 1` bound, and queue
constants 2450 and 2460 in one function.
...
request property. Both source modules consume the same exact, source-independent
predicates and the same Unity enum constants. That is why the shared support file exists
— two real consumers today. It must remain a constants-and-result-types file. A mode
parameter, a gate table, or a shared `Evaluate*` body in it would be the framework this
note refuses. Duplicating the two four-line predicates instead would also have been
acceptable — duplicating the constant table would not.

Both eligibility functions keep returning the **existing**
`LilToonOpaqueConversionOutcome` / `LilToonOpaqueConversionRefusal` vocabulary, extended
with three members: `UnsupportedForwardAddAlphaBoost`, `UnsupportedDistanceFade`,
`UnsupportedSubpassCutoff`. So the typed test seam and the preparation call sites do not
change shape. `ConversionRequestForFamily` on preparation returns a per-family
`Combine(RecipeEvidenceRequest, SourceEvidenceRequest)`. That preserves both the derived
...
shared target exist here. That meets the "two producers, one shared concept" evidence bar
of the frontend-comparison record. The *target* is the shared part — a measured fact
(§11), not a hoped-for generality. `[DECISION]`

**Second-consumer audit of every abstraction one could propose here:**
...
**Why Transparent Normal was the right next candidate:** the official target of no other
remaining family needs so little. It requires **no new attestation, no new recipe, and
no new preparation code** (§11). So it isolates the alpha-semantics question from every
other variable. That isolation made the byte-identical-pass result of §4.3 decisive.

## 15. Decisions — resolved

The controller resolved all five open decisions on 2026-09-01. Nothing in this note is
now blocking.
...
2. **P1 sequencing** (§3.4): **one combined scratch probe, authorized and executed.**
   The probe measured both digests rather than pre-registering them. `[DECISION]`
3. **Cutoff bound** (§9.2): **family-specific `<= 1` adopted**, after a repair turned the
   exact-representability claim into a sign-preservation argument. `[DECISION]`
4. **`_UseDither`** (§6 row 16): **omitted from the transparent request** — the pinned
   `LIL_RENDER 2` source compiles the path out. The design retains the positive
   `_UseDither = 1` falsifier. `[DECISION]`
5. **Module split** (§13.4): **approved**, subject to the ownership repair recorded in
   ...
   property, and `_Cutoff` moves out of the schema of the target. `[DECISION]`
6. Carried forward, unchanged and not blocking: the four open decisions of F0 §13 (OnePass
...
- The exact pass topology and identity of Transparent Normal. The structural
  counterexamples that separate it from OnePass and TwoPass (§4.2).
- `ltspass_transparent` and `ltspass_opaque` differ in exactly seven places. None of them
  touches the four referenced pass declarations or the compiled feature set (§4.3). So
  conversion preserves pass participation exactly.
...
- Three transparent-only writers have no cutout analogue: ForwardAdd premultiply,
  distance fade, and the subpass shadow clip (§5.3, §5.4, §5.6). A copied cutout gate
  list would miss all three.
- That the cutoff bound for this family is `<= 1`, not `<= 0.9999` (§9.2), by
  sign preservation rather than exact representability.
- That the colour-pass output at `a ≡ 1` is arithmetically equal to the output of the
  target (§9.1).
...
- The two transparent canonical digests are `ea247d3c…2ba13` and `700a6076…52517f`,
  stable across two sessions. AMUSE measured them with its own algorithm in an install
  that reproduced all five existing pins (§3.4) `[MEASURED]`.
- A fully-opaque Transparent Normal material and the canonical clone of AMUSE for it
  render bit-identically, shadows included (§9.4) `[MEASURED]`.
- The official target, its recipe, and its preparation code are reusable unchanged (§11).
  The attestation profile model takes a third profile without refactor (§13.2).
...
- Any behavior of the regenerated uploaded shader artifact (argued invariant, not
  digest-attested).
- Behaviour on graphics APIs other than Metal, or in Linear color space. Also behaviour
  under a dither sampler filter mode other than the installed `Point`. The §9.4
  measurement covered one configuration. The dither texture is an engine asset with
  fixed contents. The measured `Alpha8` values are exact `0`/`1`. So no colour-space or
  API dependence is expected — but that expectation is `[INFERENCE]`. Filter independence
  specifically is **not** established: slice 14 is `0 1 1 1 …`, not all-ones.

## 17. Verdict and next recommendation

**Verdict: GO — the positive contract holds for every relevant pass, and measurement
discharges both prerequisites (§3.4, §9.4).**

The transformation is sound and unusually cheap for `FORWARD`, `FORWARD_ADD`, and `META`.
The target side needs no behavioral work at all. `SHADOW_CASTER` is now measured rather
than assumed. The architecture takes one sibling module, one small shared support file,
and one enum member.

The strongest single piece of evidence is not any one digest. The **unmodified** clone
path of AMUSE took a defaults-plus-opaque-texture Transparent Normal material and
produced a canonical `lilToon` clone. The rendered output — forward shading and cast
shadow together — was bit-identical over 65536 pixels. A `_Color.a = 0.5` control
diverged on 3954 of them (§9.4).
...
2. On acceptance, write the implementation plan. No production branch until the
...
target decision. Overlay depends on this theorem plus a product decision. Lite
...
SHA-256 `34d172761c51aa9469a904704109086aafa6125a4fa0e058766e2ddc73d3b303`. Vendor VPM
index <https://lilxyzw.github.io/vpm-repos/vpm.json>. An abandoned intermediate approach
fetched NDMF `1.14.8`, and the session **removed** it before taking the measurements. No
NDMF code participated in any recorded result. A byte-identical copy of
`LilToonSourceAttestation.cs` (SHA-256
`70140d4da04ecf27d852d2519cfa88129a59a2e5ba839ef029dc585ed5783ed5`) drove the digest
measurement, not a reimplementation.
...
This session issued no Unity MCP call. The investigation enumerated two existing Unity
instances read-only and never targeted them. All Unity work ran in a throwaway project
created outside AMUSE under the OS temporary directory, addressed by an explicit
`-projectPath`. Every probe asserted `Application.dataPath` in its own output. The
process did **not** install lilToon into AMUSE, and it wrote to neither AMUSE project.

No vendor source entered the repository. The read-only clone and the scratch project
both live outside AMUSE. The session deleted the scratch project after recording its
results here.
