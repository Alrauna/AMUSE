...
**Status: proposed design, not implemented. Design-only; no production code or tests were
written.**
...
material slot, integrate NDMF persistence, acquire texture evidence, extend lilToon, run an
...
The pinned Poiyomi Toon 9.3.64 shader asset was located in a vendor package by asset GUID
and confirmed against AMUSE's production attestation constants:
...
The hash was recomputed with AMUSE's own normalization
...
Vendor source was read read-only; nothing was modified, copied into this repository, or made
a build dependency.

### 1.2 The complete canonical Opaque tuple

The vendor declares `_Mode`'s presets as ThryEditor `on_value_actions` metadata inside the
...
- **`_AddBlendOp` and `_AddBlendOpAlpha` appear in no preset's action list.** The recipe
...
The SubShader's own tags are `RenderType = Opaque`, `Queue = Geometry`,
`VRCFallback = Standard`, so a material with no queue override already reports an effective
queue of 2000.
...
1. The investigation's "when alpha clipping is inactive the threshold is irrelevant and must
   not cause refusal" branch is **vacuous for this shader**. It stays correct as a general
   rule and is deliberately not implemented, because implementing an unreachable branch
...
The vendor's declared maximum is **1.001**, not 1. A material whose `_Cutoff` sits at the
top of its own inspector-declared range discards alpha exactly 1. The investigation warned
that a declared range constrains the widget rather than what can be serialized; the stronger
...
Writing `_Mode = 0` is an identity **conditional on** the consumer's alpha ≡ 1 proof over the
same admitted states **and** on outlines being disabled. Both conditions are load-bearing.
...
`Thry.BlendOp` was read from the vendor's own `ThryWideEnum.cs` and matches
`UnityEngine.Rendering.BlendOp` ordering: `Add=0, Subtract=1, ReverseSubtract=2, Min=3,
Max=4`. So **`_AddBlendOp`'s serialized default is 4 (Max)**, and no preset action changes
it — including the Opaque preset. The recipe rewrites only the ForwardAdd *factors*, so the
op is identical on both sides of the conversion and cancels out of any comparison between
them. §5.4 records why that makes `_AddBlendOp` a non-dependency rather than a gate.

The `Add` pass also contains `if (_AddBlendOp == 4) { poiFragData.alpha =
saturate(poiFragData.alpha * _AlphaBoostFA); }`. Since the recipe writes neither
`_AddBlendOp` nor `_AlphaBoostFA`, this branch behaves identically before and after
...
**This is why enabled outlines must refuse.** AMUSE's `MaterialSemantics.Alpha` models the
base-material alpha equation. It models none of steps 2–5: not the outline mask, not the
outline texture or line-colour alpha, not the distance fade. A proof that base alpha ≡ 1
therefore says nothing about the alpha the `Outline` pass carries into its own clip. Writing
`_Mode = 0` would force that unmodelled alpha to 1 before the clip — resurrecting outline
fragments the author faded or clipped away. That is a false positive in exactly the direction
the correctness policy forbids, and it is not fixed by base alpha being exactly 1.

`_EnableOutlines` is declared `[DoNotAnimate][HideInInspector][ToggleUI] … = 0` and is used
both as the `applyOutlineColor` clip term and as a vertex offset scale
...
so applying the vendor's canonical outline blend tuple is irrelevant rather than merely
...
> must establish that the effective clip behavior is known and that alpha exactly 1 is not
> discarded.
...
weaker, and no interval, symbolic or richer-admission machinery is introduced to say it.

This is not framebuffer equivalence and must never be described as one.
...
`MaterialEvidenceRequest.Combine` over each admitted material's family alpha request — and
that single `CapturedAnimationEvidence.RelevanceRequest` is used for two different things:

1. capturing every admitted material's evidence, and
...
with an animated `material._ZWrite` would begin failing ordinary alpha analysis on state
...
yields a **conversion refusal**. Its alpha analysis result is unchanged. The refusal
...
**The queue is not animation-reachable.** Unity's material animation binding syntax is
`material.<PropertyName>`; no binding form addresses a material's render queue, and
...
This resolves the investigation's open question about render-queue override identity:
**"an override exists" is an implementation detail, not a fact this design models.**
`material.renderQueue` already resolves an absent override to the shader's declared queue.
...
is written. A test pins this directly (§8.5).
...
already admitted* evidence. The consumer's obligations are specified in §7 and implemented
there.
...
members. Validation failure is a defect (§6.2); animation is the consumer's (§7).
...
reachable. It has a production caller in the consumer's sequence (§7).
...
and would not carry conversion's schema.
...
`_EnableOutlines` is read but never written; the queue and tag are written but are not
properties.

`_AlphaToCoverage` and `_AlphaPremultiply` also appear in
`PoiyomiMaterialSemantics.AlphaEvidenceRequest`'s gate sets. That overlap is benign and
`Combine` handles it: in the composed pipeline these two cost conversion zero additional
coverage, because an attested alpha proof has already forced both to zero.

**Relevance follows the request.** Because relevance is computed from the request
...
its caller attested; in production that member is defence in depth rather than the normal
path.

### 5.3 Evaluation order

Evaluated in this order; the first failure is the reported refusal. The order is
...
and is therefore unreachable once step 2 has established that nothing will be changed.
...
7. **Depth comparison.** `_ZTest != 4` → `UnsupportedDepthComparison`. Conversion requires
   ...
   with `Always`, `Greater` or `Disabled` is expressing a visibility intent the alpha proof
   knows nothing about. The recipe still writes `4`; on an eligible material that write is a
...
9. **ForwardAdd RGB blend factors.** The `Add` pass has independent blend state (§1.3) whose
   ...
   **This is a pinned Poiyomi predicate over two named passes, not a pass model**; nothing
   here generalizes to arbitrary passes, and no pass abstraction is introduced.
10. **Clip threshold.** `_Cutoff > 1f` → `ClipThresholdDiscardsOpaqueAlpha`. Finiteness is
    already established at step 3; `_Cutoff < 0` is eligible, since α = 1 trivially survives.
...
`_AddDstBlendAlpha`), and the six `_Outline*` blend fields. Depth write and the opacity flag
are policy-authorized normalizations (§2.1); the alpha-channel fields govern the destination
alpha channel, which the compatibility contract does not constrain; and the outline blend
fields are irrelevant once step 4 has established that no outline fragment survives.
...
9 has proven that the factors the recipe *does* change are equivalent at α = 1 —
...
`ConversionRequiredSchemaProperties`, and not conversion-relevant. A curve animating it is
likewise preserved unchanged: it drives the generated material exactly as it drove the
source, through the same unchanged operation, over operands the factor predicate has already
proven identical. There is nothing for conversion to depend on.
...
override tag reading `Opaque`. No clone is created; the consumer uses the source material
unchanged.

The tag is part of the comparison on purpose. A material matching every property and queue
but tagged `Transparent` is not canonically opaque and must be cloned so the tag is fixed.
...
that is already canonical already has `_Mode == 0`; that forcing is the author's existing
state, not something AMUSE would introduce. Reporting a refusal there would claim AMUSE
...
- **Nothing is saved.** No `IAssetSaver` call, no `AssetDatabase` write.
  ...
  while NDMF's cleanup destroys only unreferenced *components and game objects* — a material
  saved eagerly and then abandoned is welded into the generated-asset container forever.
  Persistence belongs to assignment, which is the consumer's job. `PrepareCanonicalOpaqueClone`
...
- **The clone is not named.** Deterministic generated-asset naming and logical output identity
  were deferred to the vertical slice's own design. **Handoff obligation: the consumer must
  name the clone before assignment**, because container sub-asset names come from the object's
  own name and NDMF guarantees no determinism.
- **No keyword manipulation.** Recorded trap for any future work here: `IsKeywordEnabled`
  returns false for a keyword the material's shader does not declare, even when the keyword
  is present in the keyword list.

### 6.2 Validation failure is a defect

After writing, the clone is re-read through the same reader eligibility uses — a fresh
...
**A disagreement is an invariant failure, not a refusal.** Eligibility has already proven
...
material's render state is false, and that is a compatibility or programming failure, not an
...
propagates as the defect it is. Full re-attestation is not repeated — the shader object is
the same instance whose GUID and normalized hash were just verified.
...
1. **Combine requests only where conversion is attempted.** Build the renderer's closed
   request as `MaterialEvidenceRequest.Combine(<family alpha requests…>,
   PoiyomiOpaqueConversion.ConversionEvidenceRequest)` for that renderer only. Renderers not
   attempting conversion keep the alpha-only request, so ordinary analysis coverage is
   unchanged (§3.2).
...
3. **Evaluate conversion per admitted material state** with `EvaluateVerifiedEligibility`,
   supplying that state's derived conversion-relevant property evidence together with the
...
4. **Enforce the renderer-wide overwrite rule.** A `material.<Property>` curve addresses no
   ...
   already equal that property's canonical value; otherwise the applied recipe is provably
...
5. **Refuse conversion for unadmitted conversion-relevant state**, including `_EnableOutlines`,
   and keep that refusal in the consumer's vocabulary, not in ordinary alpha analysis.
...
properties are part of the combined relevance request, because those gates key on the
...
Obligation 3 above cannot be met against the repository as it stands. This was found while
writing this design; it is stated as a required condition, **not** solved here.
...
So the derived evidence conversion needs is computed and thrown away, and there is no
...
depends on, which is exactly the stale-capture hazard the repository's own guidance forbids.
...
decision (§11.1), and its natural home is the vertical slice's own design rather than this
prerequisite.

This does not block the present branch. `EvaluateVerifiedEligibility` is pure over evidence
and two scalars; it is fully specifiable and fully testable now, and remains correct whatever
...
- **All 25 canonical facts matching, `_EnableOutlines = 1` → `AlreadyOpaque`, and no clone is
  created.** The ordering regression for §5.3 step 2 and §5.5.
- All 25 canonical facts matching with a **non-finite** `_EnableOutlines` → `AlreadyOpaque`,
  and no clone is created. A transformation gate must never fire when there is no
  transformation.
- **Contrast — a gate-failing recipe property is not a canonical material.** Every canonical
  ...
  `_Cutoff`'s canonical value is 0. Evaluation proceeds to the transformation gates and the
...
- **`_AddBlendOp` does not affect the outcome.** A material eligible with `_AddBlendOp = 0`
  is equally eligible with the vendor's serialized default of `4` (Max) and with every other
  value, because conversion neither reads nor writes it (§5.4). Asserting this pins the
...
- under `PoiyomiMaterialSemantics.AlphaEvidenceRequest`, `ResolveProofRelevant` returns
  `Irrelevant` — ordinary alpha analysis is untouched;
...
**No vendor-source parser is proposed, and repository inspection found no stronger
dependency-free oracle to report.**

- The public AMUSE project's `Packages/` contains `com.alrauna.amuse`,
...
- The only in-repo Poiyomi artifact is `Tests/Editor/Semantics/Poiyomi/PoiyomiSemanticTest.shader`,
  ...
  of Poiyomi's equations or `on_value_actions`.
- Copying the vendor's `on_value_actions` excerpt into the repository would produce a second
...
recipe is safe as AMUSE-owned constants provided the two are bound together, which §8.11
does by making a pin bump without recipe re-derivation a failing test. That ships no vendor
content and downloads nothing.
...
It must be extended to declare **all 24 conversion-read properties with the vendor's exact
types, defaults and ranges**, most importantly:
...
The `1.001` bound is load-bearing for §8.7. `_AddBlendOp` is also declared — with the vendor's
default of `4` — but only so §8.6 can vary it and assert it changes nothing; it is not a
...
properties cannot change any existing test's result, since every current test reads named
...
1. **How a closed admission flow exposes conversion inputs** (§7.1). Conversion needs, per
   ...
   `SlotResolutionResult`, or introducing something else are all open; none is chosen or
   designed here, and none is implemented on this branch.

Both decisions previously listed here are resolved: `_AddBlendOp` is removed from conversion
entirely (§5.4), and `AlreadyOpaque` is now classified before the transformation gates
(§5.3 step 2, §5.5).

### 11.2 Corrections to the investigation

1. **§2's tuple table is incomplete.** It lists 9 fields per preset; the Opaque preset carries
...
2. **§2's `_Mode` runtime claim is incomplete.** The opaque branch body is commented out
...
3. **§7's "clipping inactive" branch is vacuous for this shader** (§1.4). The general rule
   stays correct; no code implements it, because the branch is unreachable.
4. **The declared cutoff range is itself unsafe** (§1.5).
5. **§7's admission list is insufficient.** It requires attestation, the base blend predicate,
   premultiply off, coverage off and clip proven — but not disabled outlines, not `_ZTest`,
   and not the ForwardAdd blend factors. All three are added here.
6. **§10's render-queue-override question is resolved as an implementation detail** (§3.4).
7. **§10's `AlreadyOpaque` question is resolved as a successful no-op** (§5.5).
...
- **Per-property interchangeability for animated recipe fields** — the consumer's known
  conservative refusal (§7).
- **Outline alpha modelling.** Deliberately not started. Supporting enabled outlines requires
  ...
  the distance fade — a texture-evidence-dependent expansion of `MaterialSemantics`'s alpha
  domain, gated behind the unstarted texture-evidence investigation.
...
- **The vendor's `_BSSEnabled` toggle carries its own `on_value_actions`** writing
