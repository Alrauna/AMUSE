...
formats. Real avatar textures are none of those things.

Every texture-backed opacity proof therefore fails on real content today, which is why the
...
Each claim below comes from the checked-out tree at `cff5025`.
...
Two observations that change the scope of this design:

**The instance half of `UnityAlphaFieldEvidence` has no production consumer.**
Only `Tests/Editor/Host/UnityAlphaFieldEvidenceTests.cs` and
`Tests/Editor/Host/AlphaEvidenceClassifierIntegrationTests.cs` reference
`new UnityAlphaFieldEvidence(...)` and `TryGetAlphaField`.

The two production providers are the lambdas at `UnityRendererAlphaAnalysis.cs:503`
and `AmusePlatformFinishPlugin.cs:468`, both built over `GatherAlphaFields`. The
only production entry into the class is the **static** `TryCapture`.

The instance half of Seam 3 is therefore a test-only surface today. This design
changes its types so it stays compilable and honest, and does **not** delete it: removing a
...
We evaluated and rejected the alternative — beside `AlphaTextureData` in `TriangleAlphaClassifier.cs`.
That file is the **single-grid classifier vocabulary**:
`TriangleAlphaOutcome`, `AlphaFilterMode`, `AlphaWrapMode`,
`AlphaSamplingSettings`, `TriangleAlphaInput`, `AlphaTextureData`, and the
classifier itself, 610 lines.

The classifier never sees a chain. The pinned
boundary keeps both it and `AlphaTextureData` unchanged. Adding the chain there
would place a type inside the one file this milestone must leave alone,
and it would imply the classifier consumes it.

We also evaluated and rejected `AlphaSemanticsResolver.cs`: `Editor/Host/` types
(`CapturedTextureEvidence`, `GatherAlphaFields`) carry the chain. They have
...
change in the sense that matters: it touches no pinned file.
...
- no `Width` / `Height` — those belong to a level, and a chain-level dimension
  would be the mip 0 dimension under another name.
...
The constructor copies the list into a private array. `AlphaTextureData` already
makes each element immutable and already copies it defensively, so a shallow copy
...
`ProvenOpaque`" vacuously true and turn the conjunction in §5.3 into an unconditional
`ProvenOpaque` — the exact false-positive direction that the correctness policy of AMUSE
forbids. Making the empty chain unrepresentable is why this is a type and not an
`IReadOnlyList<AlphaTextureData>`.

Invariant 4 applies per axis because §6.3 of the investigation measured
...
The producer enforces it and refuses the whole texture unless every
...
It has no way to represent a colour, a magnitude, a non-alpha channel, a 3D or array
texture, or a level in any form other than the existing grid.

It does not extend into a sampling framework without adding every one of those.
It exists because §6.1 of the investigation measured a texture whose mip 0 proves
opaque while mip 1 does not, and the current single-grid value cannot express the
counterexample.

## 5. The six seams

Each seam carries exactly one `AlphaTextureData` today. The change is a change of
type on one value, propagated. To avoid it, the design adds no adapter, no second
provider, no compatibility shim, and no parallel evidence graph.
...
2. A new clause: the chain is the **complete declared mip chain of the source in
   order**, mip 0 first.
   Without this the conjunction in §5.3 is unsound — an
   unexamined level the sampler could select could be non-opaque, and the
   resolution would report `ProvenOpaque`.

predicate" stays true and gains a clause: hardware chooses the level, the choice
is not knowable, and the conjunction over levels is what closes the gap. Trilinear
...
- the parameter of the private constructor changes type. Its null guard at `:67-70` keeps
  its meaning — a classified resolution still always has its evidence.
...
`ProvenOpaque` is reachable only after the loop body executes at least once,
which invariant 2 of §4.3 guarantees.

### 5.4 `TryGetUniformOutcome` stays exactly as narrow

The design introduces no new uniform path. In particular the tempting shortcut — "every
...
carrying the same outcome **merge**.

Converting classified evidence into a uniform resolution therefore changes merging
behaviour, and it re-describes a classified chain as uniform on the strength of a
property of the evidence — the same door the comment at
`AlphaSemanticsResolver.cs:125-134` closes for sampled agreement.

The existing `IsFullyOpaque` short-circuit inside the classifier already delivers the
performance benefit without touching the kind of the resolution (§14).
...
The lemma keeps its form and, if anything, gains strength. For `k < 1` the resolver
needs the provider to attest that the sampled value lies in `[0, 1]` and not
one byte of its contents. Under a chain that attestation holds at **every** level.
Bilinear filtering within a level is a convex combination, so whichever level the
hardware selects, `alpha = s * k <= k < 1` at every reachable sample.

The premise of the multiplier lemma now holds over a larger set of sampling
...
resolutions — including with themselves — still holds for the same reason.

The comment updates to name `AlphaMipChain`, and gains nothing else. This is the only
change to that file.
...
We inspected packaging before choosing this. `.github/workflows/release.yml`
...
`Tests/` ships and any path inside it cannot.

`Editor/Host/Shaders/` therefore
ships with the product. Placing it under `Editor/` also means Unity
...
The design excludes the research package from the VPM listing and from every release
...
install shape, and it is the mechanism the merged research probe already
exercises.

We considered and rejected `Shader.Find`: it resolves by shader **name**, which
this repository does not own and which can collide with a `Hidden/` shader of
another package, and it would silently bind to whichever asset won.
We also considered and rejected GUID lookup via `AssetDatabase.GUIDToAssetPath`:
...
package, and the move does not change the product `package.json`.
...
Lookup is by path, so preservation is insurance rather than a requirement.
Moving the `.meta` is mandatory in either case: a missing `.meta` gives the asset a
new GUID on import, which `.omp/RULES.md` rule 4 treats as prohibited GUID churn rather
than a cosmetic change.

**The move does not change the shader body.** It keeps
...
component**. The destination discards the green raw-alpha component before
...
samples, validates, or branches on it.

The research characterization renders the
same shader into its float diagnostic target and reads green there, which is why
the move preserves the channel instead of deleting and re-creating it.

Only the header comment changes: from "research-only, never referenced by the
product package" to text that names the production role of the shader, its `R8_UNorm`
...
could emit red alone. **That is withdrawn.** It would create a second asset
...
`TryCaptureRawAlphaDiagnostic` through its float diagnostic target.

The one case that genuinely needs magnitudes —
`AFloatFieldDefeatsTheExactlyOnePredicateAsAnAttestation`, which shows the probe stores `2.0`,
`-1.0`, `NaN` and `+Inf` while the one-bit predicate reports the same
...
One route for every admitted format. Production removes `GetPixels32`.
...
`useMipMap = false`, and `autoGenerateMips = false`.

Call `Graphics.Blit(texture, target, material)`. Call
`AsyncGPUReadback.Request(target, 0, R8_UNorm)` and `WaitForCompletion()`.
Validate, copy bytes into a managed array, and construct one `AlphaTextureData`.

After every declared mip produces a level, and only then, construct the
`AlphaMipChain`.
...
The core runs exactly the Blit / `R8_UNorm` / readback / validation sequence from
...
1. **ordinary chain capture**, which calls it once per declared mip after
   the gates of `TryCapture` pass.
2. **the host-capability check of §8.2**, which calls it once against a small
   asymmetric in-memory texture.

This layering is necessary, not stylistic. The self-check builds its fixture in memory
and therefore has **no asset identity**, so `UnityTextureEvidence.TryGetSourceId`
fails for it and it can never pass gate 2.

A self-check written against the public
...
Each helper remains one narrow named check that production calls. The design
creates no general validation API, validator registry, or exported "GPU capture" surface,
...
- The constructor of `AlphaTextureData` takes `IReadOnlyList<byte>`, which
  `NativeArray<byte>` does not implement, so the existing type forces the
  managed copy rather than the design choosing it. That is the desired outcome regardless: the
...
raised from this code is a programming defect and must propagate, per `.omp/RULES.md` rule 6.
The `catch (ArgumentException)` that currently wraps `GetPixels32` disappears with
`GetPixels32`.

The design avoids console noise structurally rather than by catching: it evaluates
the format allowlist **before** any GPU call, so production never enters the
compressed-source direct-readback path that Unity logs a hard error for
(§3.2 of the investigation).
...
claim made here. The table is *not* a strict cost ranking — the asset-database

identity lookup of gate 2 costs more than the enum comparison of gate 3, and it
deliberately precedes it, because identity is the precondition for the whole
evidence contract and the design refuses a texture without it regardless of target. The `Cost` column
...
target whose import this design characterized, and on a non-Windows target the loaded
...
handled**: supporting it would mean designing against behaviour nobody observed.

**Gate 8** is the precondition for the whole route: without `supportsAsyncGPUReadback`
there is no way to get bytes back at all. §2 of the investigation recorded it
`True` on the measured host, but it is a device capability, and the design must gate
it rather than assume it.

**Gate 10 is not a duplicate of gate 4.** Gate 4 asks a policy question — is this
`TextureFormat` one AMUSE characterized — over the enum Unity reports for the
imported asset. Gate 10 asks a device question about the source the shader will
`Load`, so a source the device cannot sample refuses *before* the design allocates
...
load because native `RGB24` support is rare, and measurement showed the production
route samples `RGB24` alpha as exactly one everywhere, at 4x4 and 8x8, single-mip and
mipmapped.
...
We rejected `GetCompatibleFormat(...) != GraphicsFormat.None` as a general
...
exists. Measurement found every other admitted format exactly sampleable, so the
...
supplied by `SourceSamplingGatePasses` — and the design writes them that way. That is acceptable, and is the preferred shape, because
production calls the predicate: the predicate *is* the gate, not a
copy of it (§13.4).

Gate 12 is last because it needs gates 9 and 11 to pass first: the
check itself renders through the production shader into an `R8_UNorm` target.

The channel gate lives separately, in `TryGetAlphaField`, where it lives today:
...
cannot attest finite-and-`[0,1]`, and the format supplies no structural guarantee).
`DXT5Crunched` behaves as `DXT5` in an earlier measurement, but the R8 path does
not durably exercise it. The design refuses `ARGB4444`, **not** because 4-bit alpha
quantizes several authoring values to exactly one — that alone is not unsafe.
The imported GPU-decoded representation is what playback samples, so a source
`254` that imports to exactly one genuinely *is* opaque as rendered. The reason is
different: that format has no durable approved production-shaped characterization in
this milestone.

The design also refuses all ASTC (tolerance-based decode, and it is the Quest
format). It refuses ETC/EAC, PVRTC, BC1/BC4/BC5, and everything unlisted (not
characterized). It refuses every non-`StandaloneWindows64` target, streaming
textures, nonzero mip limits, and non-alpha channels. It refuses a host missing
async GPU readback or exact `R8_UNorm` render/readback, and an **alpha-bearing**
admitted format whose exact reported `graphicsFormat` the host cannot `Sample` —
`RGB24` is the one measured exception, admitted because it carries no alpha channel
and Unity converts it to `RGBA32` at load (gate 10 above). It refuses a host whose
§8.2 capability check did not pass, and any partial or malformed GPU result.
...
`false`, resets `source` and the chain out-parameter, and constructs no chain.

Two structural properties make this leak-free and partial-evidence-free:

1. A level-scoped `finally` releases each `RenderTexture`, and the texture-scoped
   `finally` releases the material, so a mid-chain failure releases everything
   already allocated and allocates nothing further.
2. The design constructs the `AlphaMipChain` **only after** every declared mip
   succeeds. There is no code path on which a partially populated chain exists,
   so none can escape. Levels captured before the failure are ordinary managed
   objects, and garbage collection reclaims them.

A failed alpha capture does not widen refusal for the other facts of the texture: see
§10.
...
#### Why it exists
Row order is soundness-critical: rows of `AlphaTextureData` follow
`y * Width + x`
...
nothing about the graphics API of the editor, and capture runs in whatever Editor the
user builds in.
...
- **Fixture.** A small in-memory `Texture2D` in an admitted uncompressed format
  ...
  result from the expected one. The check creates, uses, and destroys it inside
  itself. It writes no asset, and nothing persists.
...
- **When.** Lazily, **once per Editor AppDomain**, on the first capture that
  reaches gate 12 — that is, after gate 11 loads the production shader and
  gates 8-10 confirm the device capabilities the check itself depends on. A
...
- **Storage.** A `static bool?` on `UnityAlphaFieldEvidence`. Domain reload clears
  it naturally. It is a **process-local host-capability latch**: it records one
  fact about the graphics stack of this Editor process, nothing keys it, and it holds
...
**The design deletes only `texture.mipmapCount > 1 ||`.** The doc comment drops
...
What makes admitting mipmaps sound under `Bilinear` is that `Bilinear` in Unity
...
classifier sound for mipmapped textures (§6.1 of the investigation shows a
texture whose mip 0 alone proves an opacity that mip 1 refutes).

## 10. Capture reuse

**The design introduces no texture-evidence cache** — no source-scoped, build-scoped, or
static store of captured alpha evidence, and no registry or service. (The §8.2
host-capability latch is not one: it holds a single boolean about the graphics stack of
this Editor process, nothing keys it, and it stores no texel, texture, or
source identity.) The existing deduplication in
`UnityMaterialEvidenceCapture.Capture` (`:674-721`) is the whole mechanism:

1. the design resolves the texture of every assignment to a `TextureSourceId` and registers it in
   a batch-local `Dictionary<TextureSourceId, SharedTextureBuilder>`.
2. the design unions requested evidence kinds per source.
3. `CaptureTexture` runs **once per distinct source for the whole batch**.
4. the design hands the single resulting `CapturedTextureEvidence` to every assignment for
   that source.

Code reaches `TryCapture` — and therefore all GPU work — only from `CaptureTexture`
...
at gate 2 (`TryGetSourceId` fails) before allocating anything. The design refuses
an unidentified texture cheaply, every time.

**Honest limit.** Deduplication is **batch-scoped**, and the caller sets the batch
breadth. `UnityMaterialSemantics.CaptureAlphaMaterials` batches all materials of a
renderer into one `Capture` call, so the design captures a texture shared across
the slots of a renderer once. `TryAttestAlphaMaterial` uses a single-material
batch. A texture used by two renderers is therefore captured twice.

This milestone accepts that cost: a build-scoped texture-evidence cache is explicitly
out of scope, and
...
simply never enters the dictionary of the provider. The provider returns `false` for it,
...
unaffected, and the sampling and colour facts of the texture remain available to any
...
**The design requires no NDMF phase or plugin change, and it proposes none.**

`AmusePlatformFinishPlugin.Configure` (`:91-101`) registers two passes in
`BuildPhase.PlatformFinish`, and the second is `sequence.Run(BarrierPassName,
...
`AsyncGPUReadbackRequest.WaitForCompletion` are all main-thread Editor-legal.
`WaitForCompletion` makes the capture synchronously complete before the design hands
the immutable evidence onward — which the existing architecture requires,
since the pipeline consumes `CapturedTextureEvidence` after `Capture` returns and it must hold
no pending work.

Two lifecycle facts worth stating rather than assuming:

- The pass reads the materials of the **build avatar**. NDMF clones materials but not
...
- The capture blocks the Editor main thread while the readback runs.
  §5.4 of the investigation observed 17-56 ms per 2K/4K chain, single-run and
  illustrative. This is build-time cost, not a correctness concern, and §14
  describes how to observe it.

The route writes no mesh, submesh, material, importer, quality setting, streaming
state, scene, or prefab. It reads GPU state through a temporary render target it
owns and releases.
...
`MaterialSemantics`, `package.json`, every `.asmdef`, and the phase configuration of the
plugin.
...
package. The design creates no shader and deletes none. Expected `.meta` changes are
...
**one moved**, the `.meta` of the shader itself, carrying GUID
...
RED first for every behaviour that is **actually constructible**. Where the design
cannot reach a branch without synthesizing a Unity failure, this section says so
explicitly rather than promising coverage it cannot deliver.

Two rules bound what the design may add:

- **The design allows narrow pure helpers**, and they are the preferred shape for validation
  ...
  helper** — the helper *is* the production check, not a parallel copy of it. A helper
  that production stopped calling would be a test-only seam and would
  have to go.
- **No injectable GPU backend, interface, factory, seam, or test-only production
  framework.** The design adds none of these to synthesize Unity failures. A branch
  reachable only by faking Unity stays unautomated, and the design reports it
  (§13.5, §13.6).

Real integration coverage — meaning tests that drive actual Unity textures through
the actual production path — covers successful `R8_UNorm` acquisition, every
admitted format, non-readable compressed multi-mip evidence, orientation, full-chain
capture, the publicly reachable refusal gates, and restoring observable state.
...
**Representative is deliberate.** The production-called `TextureFormat` predicate
covers the complete allowlist/refusal policy exhaustively below, so manufacturing
an ASTC or `DXT5Crunched` fixture merely to watch an enum comparison reject it
before any GPU work happens gains nothing.

An earlier draft of this design claimed the merged research fixtures already
exercise those two
...
**Covered by production-called pure predicates**, because the design cannot safely induce the
deciding facts on a conforming host or without mutating project state. Each is
...
hook to simulate one.** The guarantee is control flow: the result of the check assigns the
latch exactly once, and gate 12 reads it before every
capture. Review confirms both. No test induces either.

The design asserts the channel refusal where it lives, on `TryGetAlphaField`:
...
The design cannot provoke any of these conditions on a conforming host without faking Unity,
and this design forbids the seam that would allow it. Coverage lives at the honest
level: the pure validation helpers of §7.2, each of which production
calls:
...
These comparisons catch a defect even though the design cannot stage the Unity condition
that would trigger it. That is the whole value of the helper shape, and
...
the constructor of `AlphaMipChain` "rejects every shape a partial chain could take."
...
- **Completeness belongs exclusively to the provider contract (§5.1) and the
  ...
  design ever reaches the constructor.
...
- **Mid-loop failure producing no partial result** remains a **structural
  ...
  constructed. No test fakes a failed level, and no doc may describe the
  constructor as providing this.
...
has no reliable way to observe the retained contents of that pool or of
ownership within it. An assertion about pool residency would test an unverified
model of Unity rather than AMUSE.

**The design writes no resource-leak test for a gated refusal.** Gates 1-11 all precede
the first `GetTemporary` of the caller, so the code has allocated nothing that can leak. The
...
This design identified no deterministic, safe, real trigger for the latter two. A readback
...
The design records these as unautomated branches rather than papering over them.
...
**The design proposes no new budget, and no new metric, counter, telemetry type, or
reporting surface.** The investigation withdrew its `4/3` cost
estimate as unjustified, and nothing replaced it with evidence.
...
Two mitigations already exist in the code. Use them before contemplating
anything new:
...
How to observe it during implementation, without new infrastructure:
...
2. if that shows pressure, add a **throwaway** probe that counts classifier
   ...
   probe** once it captures the conclusion.
...
Not in this design: mesh or submesh mutation, and generated opaque materials.
Also not in this design: any render-state conversion change (that capability is
merged and is not reopened), vertical-slice orchestration, and NDMF phase or plugin
changes. Also not: lilToon- or Poiyomi-specific texture rules, assigned Poiyomi
`_AlphaMask` semantic support, and any texture importer mutation. Also not:
Android/Quest, `DXT5Crunched`, a universal texture, material, or shader IR, and a
generic GPU extraction framework. Also not: any source- or build-scoped
texture-evidence cache, a new planner, and RGB or any non-alpha channel. Also not:
trilinear, anisotropic, derivative, LOD, or streaming sampling semantics, and any
Census Lab work.

Also deliberately not done, though adjacent:

- **The design gives `ResolveScaledSample` no chain-aware fast path.** It reads no
  contents today and will read none after.
- **The design does not delete the instance half of `UnityAlphaFieldEvidence`**, despite
...
- **The design does not relax `mipMapBias != 0f`**, though §9 shows the conjunction now
  covers it. Conservative deferred coverage — a coverage defect worth reporting,
  not fixing here.
- **The design does not admit Trilinear**, though §9 withdraws the claim that a per-level
  conjunction cannot cover it. Admitting it requires widening the
  `TextureSampling` / `AlphaFilterMode` vocabulary and the admitted
  modes of the classifier, which is a separate milestone. Conservative deferred coverage, recorded
  alongside mip bias.
- **The design does not generalize the §8.2 latch.** It stays one nullable static and one
  private method, and the design never turns it into a capability service, a registry, or an
...
evidence. This design does not edit it retroactively. The following of its conclusions no
...
1. **The analysis GPU is not the playback GPU.** Every measurement is Apple M2 /
   Metal. The specification defines `BC7` decompression as bit-accurate. `BC3` alpha is an exact
...
2. **One graphics API carried the row-order measurement**, which is why the §8.2
   host-capability check is part of this design. The build-target gate does not
   constrain the graphics API of the editor, so the check reduces this from an
   unverified assumption to a checked precondition **on the host that runs the
   build**, whose failure mode is refusal. What remains is narrower, and the design
   must not overstate it: the check attests orientation and binary `R8_UNorm` encoding on that
   host, not the decode or swizzle behaviour of each admitted compressed format
   (risk 1).
3. **The design refuses mipmap streaming and never handles it.** No one provoked what a `Load` of a
   non-resident level returns, and the design knows no read-only signal that
   distinguishes a resident level from an evicted one.
4. **The design refuses nonzero `activeMipmapLimit`**, and no in-memory fixture
   could construct that branch without mutating project state, so a pure
   predicate covers it alone.
5. **The design can observe only the import of the active build target.** A proof obtained under
   `StandaloneWindows64` says nothing about the Android variant of the same asset.
6. **Cumulative multi-mip classification cost remains unmeasured** (§14).
7. **Deduplication is batch-scoped** (§10), so the design captures a texture shared
   across renderers more than once per build.
8. **Texel-fetch transfer and swizzle semantics vary across graphics APIs**, which
   ...
   "alpha is the only channel characterized", not on knowledge that texel fetch is
   transfer-free.
9. **The §8.2 latch evaluates once per AppDomain.** A graphics-stack change
   within a single Editor process would not re-trigger it. Unity does not change
   graphics API without a restart, which clears the AppDomain, so the design judges
   this safe — but it is a judgement, not a measurement.
