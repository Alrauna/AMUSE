...
Design spec. Dated 2026-09-20. Branch `investigate/poiyomi-locked-materials`. This spec makes no production change. Controller direction, recorded 2026-09-20 after design review: transient unlock is infrastructure for the long term. The project's future transformations rely on Thry's locking as the re-protection mechanism, within its documented limits.
...
- Failure of the batch or of a pair: the slot references revert to L for the affected pairs, the U clones are destroyed, and `TransientUnlockRelockFailed` is recorded per affected slot. The shipped state equals the 2026-09-19 behavior for those slots.
...
- Census note. The research census mirrors declaration order, so each enum addition carries a census row append.
- Consent. The feature ships consent-gated per build until verification tasks V2 and V4 pass, then the gate moves to the D8 pattern: attested combinations run without per-build consent, unattested combinations ask. The consent subject names, in neutral language: the unlock window, the active re-lock, the render-equivalence basis, the fallback behavior, and the fact that the upload-time lock is not load-bearing for swapped materials.
