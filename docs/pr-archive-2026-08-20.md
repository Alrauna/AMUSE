...
Pull request titles and descriptions from before this repository was recreated on
2026-08-20.
...
Kept because these descriptions carry the per-pull-request validation evidence — test
counts, Console error counts, and private-testbed attestations — that exists nowhere else
in the repository. The designs themselves are recorded under `docs/superpowers/specs/`,
and the code history is intact in git.
...
Design-only branch establishing the architecture for AMUSE research tooling, with the avatar alpha census as the first concrete instance. **No production code, no package creation, no visibility change, no Unity change.** One file added.
...
Harness source is first-party public code in this repository. The Lab holds no source and may be deleted and recreated at any time — if deleting it would lose anything but private data and third-party packages, something is in the wrong place.

An earlier revision assumed the harness should live outside the repository as a disposable private tool. That assumption is recorded as invalidated in §0, along with five others that followed from it.
...
Unity compiles only `Assets/` and `Packages/`. `Tools/` is correctly reserved for non-Unity scripts and already holds one; C# there would never compile. `Assets/` cannot be referenced across Unity projects, so the Lab could not consume it without copying. A second embedded package is the only viable home.

The name is deliberate and is not to be changed to `tools` or `devtools`: a package named for what it *is* invites contributions with a purpose, while one named for what it *is not shipped as* invites anything with nowhere else to go. §3.2.1 sets five governance rules — first-party, never shipped, never private data, never a dumping ground, and every addition states its AMUSE purpose.

**Release safety needs no work.** `release.yml` builds `Packages/${{ vars.PACKAGE_NAME }}` and `build-listing.yml` passes `--current-package-name`. Both scope to one named package, so a second package is already excluded from release and the VPM listing. Embedded packages are auto-discovered, so `Packages/manifest.json` needs no entry either.
...
Only Collect touches Unity or AMUSE internals. Anonymize and Aggregate are pure functions over plain records, mirroring AMUSE's own analysis/mutation testing boundary.

This is what makes **non-leakage provable rather than promised**: seed tier 1 records with distinctive fake creator names, paths, and GUIDs, run Anonymize, and assert none appears anywhere in tier 2. Deterministic, needs no avatar, runs in CI. The earlier design could only ask the operator to be careful.
...
Reflection was a consequence of an externally hosted harness, which cannot be a friend assembly. With the harness in-repo that reason is gone, and reflection would cost a surface-compatibility probe, name-keyed enum handling, and the loss of compile-time safety — machinery whose entire purpose is to re-create at run time, imperfectly, what the compiler does for free.

For a tool whose value is trustworthy counting, compile-time coupling is a feature: a rename that breaks the census should break loudly in CI rather than silently in a private run.

## InternalsVisibleTo deferred

The friend grant `[assembly: InternalsVisibleTo("Alrauna.Amuse.Research.Editor")]` is justified in §4.2 but **deliberately not added here**. It lands only when the collector needs it.

It changes no public API, adds no production code and no escape hatch, and matches the existing `Alrauna.Amuse.Tests.Editor` precedent exactly. Public API promotion was rejected: it would commit AMUSE to supporting an analysis API long before the product is ready to.

## Preserved architectural constraints

- No AMUSE analysis change made solely to ease measurement.
- Unknown attribution stays a measurement limitation. The census will hit submeshes where `Unknown` triangles carry no recorded reason; the harness measures the *size of the blind spot* rather than motivating a change to production analysis.
...
It needs no Unity objects, no AMUSE internals, and no visibility change, so it lands the trust-critical half with full public test coverage and zero coupling to the production package. `feat/census-collector` follows, and that is where the friend grant happens.

## Validation

No markdown linter is available in this environment, so equivalent structural checks were run: code fences balanced, all 5 tables well-formed, no placeholders, no trailing whitespace, heading spacing correct. All 23 `§N.M` cross-references resolve against the 40 headings. The analysis surface the census depends on was verified mechanically against source at `711a7a8` — 18 members and 4 enum cardinalities.
...
No NDMF execution or source mutation is performed.
...
Unknown or unsupported information is preserved.

Missing/non-finite UV0 is treated as missing evidence rather than an automatic
failure:
...
The public development project installs neither vendor shader package, so the
integration fixture substitutes only vendor source attestation through the
existing verified-material test seam. It uses genuine Poiyomi semantic
...
- document the missing once-per-machine VPM repository setup required to discover NDMF;
- document deterministic per-clone VPM dependency restoration before the existing NDMF standalone bootstrap;
- align `Packages/.gitignore` with the state produced by the VPM resolver so clean-clone restoration does not dirty the repository;
- record the clean-environment reproduction and validation of the setup.
...
but a fresh machine could not discover that package unless the developer had already added the bd_ VPM repository to machine-global VPM settings.
...
`vpm list repos` was also observed to normalize/re-cache machine-global VPM settings, so repository inspection is not assumed to be byte-level read-only.
...
Unity MCP was not connected during final validation, so the public Unity project could not be positively identified through its `Application.dataPath` and the Editor-level V8 check was not run.

No Editor package-resolution or compilation result is claimed by this PR.

The clean-clone VPM restore and NDMF bootstrap were validated independently and successfully.
...
AMUSE development moved from Windows to macOS, exposing that previous agent
workflows had implicitly relied on the old Windows checkout path.
...
- no tracked file is accidentally ignored;
...
Each admitted format was measured against actual shader sampling during the design phase.
...
Tests establish that it follows import results for settings that change the field, including `alphaSource` and resize behavior, while avoiding unnecessary import-history reasoning.
...
`Texture2D.isReadable` is required by this first implementation.

Readability is disabled for many ordinary imported avatar textures, so this milestone intentionally prioritizes exactness and architectural validation over broad real-world coverage.

Non-readable texture evidence is deferred rather than approximated.
...
`Complete(Unmodified)` when `_BumpMap` was unassigned before evaluating its
existing `NormalFeatureGates`.
...
- no adapter interface, registry, schema, expression DAG, shared diagnostic
  framework, or third shader frontend was introduced.
...
- add source attestation for lilToon's generated-shader model using measured canonical shader/pass digests, the complete lilToon-owned include-tree digest, live `LIL_RENDER`, and compile-time feature evidence;
...
Task 0 additionally validated attestation against a real isolated Unity 2022.3.22f1 installation of jp.lilxyzw.liltoon 2.3.4 using both default and stripped shader-generation settings.

## Known limitation

The public AMUSE test project intentionally does not install lilToon. Therefore the live `GatherSourceEvidence` filesystem/package path is validated by the isolated Task 0 real-package measurement rather than by an in-repository automated integration test. Canonicalization, identity verification, failure cases, render-mode scanning, feature scanning, and semantic interpretation are covered deterministically in the repository suite.
...
Bridges normalized `MaterialSemantics.Alpha` to the existing exact triangle alpha classifier with one shader-independent, Editor-only decision function. `Resolve` returns exactly one of: a uniform triangle outcome, an exact classifier configuration, or a named material-level refusal.

## What it does

- **Shader-independent bridge.** `AlphaSemanticsResolver.Resolve(SemanticOutput<ScalarSemanticValue>, AlphaFieldProvider)` switches exhaustively over the closed Alpha vocabulary. It names no shader, property, package, version, Unity object type, mesh concept, or render state, so a future lilToon adapter uses the same path unchanged.
- **Constant fast paths.** A constant alpha is independent of UV, geometry, sampling, and texture identity, so it bypasses the classifier entirely: exactly `1` is `ProvenOpaque`, anything below `1` is `MustRemainTransparent`, and neither consults evidence.
- **Exact multiplier lemma.** For `alpha = s * k` with the evidence contract bounding `s` to `[0, 1]` (a bound bilinear filtering preserves, being a convex combination):
  - `k == 1` preserves the classifier's own `s == 1` predicate — delegate unchanged;
  - `k < 1` (including `0` and negatives) forces `alpha <= max(0, k) < 1` at every reachable sample, so the outcome is `MustRemainTransparent` proven from the range attestation alone, without reading one texel;
  - `k > 1` would require proving `s == 1/k`, which the classifier cannot express, and would leave `alpha` above `1`, whose opacity meaning the semantic model deliberately does not define — refused.
...
- **Predicate-equivalent scalar evidence.** `AlphaFieldProvider` is a lookup over `(TextureSourceId, TextureChannel)`. Its contract is stated as the proof the classifier consumes — base-level texel domain bottom-to-top, every effective per-texel scalar finite and within `[0, 1]`, byte `255` marking exactly the texels whose value is exactly `1`, every other byte strictly below `1` — not as a storage format. A source therefore need not literally be an uncompressed 8-bit `b/255` field; monotone hardware decode such as sRGB leaves the partition, and the predicate, unchanged.
- **Arbitrary RGBA channels.** Because the classifier never interpreted its bytes as "the alpha channel", Red/Green/Blue/Alpha all work through the same normalized evidence with **zero** classifier change.
- **Exhaustive sampling mapping.** Semantic `Point`/`Bilinear` and `Clamp`/`Repeat` map onto `AlphaFilterMode`/`AlphaWrapMode` explicitly, with a fail-closed default arm so a future semantic mode cannot fall into a wrong classifier mode.

## Conservative restrictions

- **UV0 with identity scale/offset only.** The classifier's proof is exact relative to the UV floats it is handed and has no transform input. Pre-computing `uv * Scale + Offset` in `float` would make it prove an exact statement about a domain off by up to one ulp, so non-identity transforms and non-zero UV sets refuse. The deferred extension must prove exactness with exact dyadic/rational arithmetic — wider floating point is not such a proof.
- **Refusal vs. `Unknown`.** Material-level refusals (`SemanticsUnknown`, `UnsupportedMultiplier`, `UnsupportedUvMapping`, `UnsupportedSampling`, `MissingTextureEvidence`) stay distinct from the classifier's per-triangle `TriangleAlphaOutcome.Unknown`. A refusal yields no triangle outcome at all — `Classify` throws — so it can never be misread as a result, and `AlphaResolution`'s constructor makes an invalid shape unrepresentable.
...
The **live Unity texture-evidence producer** is a separate milestone. The resolver is complete and fully tested but has no production data source until host extraction lands — the same sequencing the classifier and separation planner followed. A future producer may normalize trustworthy source evidence into `AlphaTextureData` for analysis; that is proof normalization, never texture baking, and never reaches the rendered result.
...
- Delegation-equivalence tests assert the resolver's per-triangle result equals a direct `TriangleAlphaClassifier.Classify` call across all four sampling combinations, pinning it as a configurer rather than a second classifier. Classifier `Unknown` for degenerate geometry, absent UV, and the workload cap passes through unchanged.
...
Editor-only **Poiyomi material semantics adapter** for AMUSE. Given a material whose shader is attested as the pinned **Poiyomi Toon 9.3.64** — verified by exact shader-asset GUID, package name/version, and normalized source hash — it produces normalized, proof-first semantics for four outputs: **BaseColor, Alpha, Emission, Normal**.

The adapter is conservative by construction:

- Anything it cannot prove behavior-preserving **fails closed to `Unknown`** with an **output-local diagnostic**; the four outputs are invalidated independently.
- Uncertainty never widens optimization — only proven forms are emitted, and refusal is always the safe default.
...
- Focused EditMode tests for source attestation/identity, texture evidence (identity, UV, sampling, color space), BaseColor, Alpha, Emission, Normal, and adversarial cross-output behavior.
- An **original, schema-only fixture shader** stands in for Poiyomi's property surface so tests run without installing Poiyomi. **No Poiyomi source is vendored** into this repository.
- Focused Poiyomi suite: **232/232 passing** (0 failed, 0 skipped); full EditMode suite green; console clean (0 errors, 0 warnings).

## Repository tooling

Adds **`CLAUDE.md`** containing only `@AGENTS.md`. This is repository agent-harness configuration: it delegates Claude Code's instructions to the existing canonical **`AGENTS.md`** policy rather than duplicating it, so the harness automatically inherits the project's agent rules.

## Scope / safety notes

- `.gitignore` is intentionally **not** part of this PR.
- The distributable package stays independently usable; no Poiyomi or MCP dependency is added to package metadata.
- Known coverage limit: locked/optimized Poiyomi materials carry a renamed shader identity and are refused (a safe false-negative) until unlocked.
...
The previous identity described only the alpha-material optimization subsystem. AMUSE is the broader proof-first material-understanding and simplification project, while the implemented classifier and planner remain valid early components of that architecture.
...
- all 52 tracked Unity GUIDs were preserved with no duplicates.
...
- No private-avatar or private-testbed content is included.
- Semantic expectations are authored independently and are never derived through Unity sampling.
- The JSON catalogs are designed to remain portable to a future Blender/Python implementation.
- `Unknown` is reserved for inputs where a safe supported classification cannot be established. Known partial transparency remains `MustRemainTransparent`, while malformed fixture data is rejected by integrity validation.
...
Sets up the minimum local development and EditMode test infrastructure needed to begin NDMF development.
...
Documents and adds an idempotent bootstrap for NDMF 1.14.4’s standalone dependencies.
Keeps generated dependency DLLs ignored and untracked.
The bootstrap is a temporary workaround for NDMF 1.14.4 packaging its standalone assemblies under Unity-ignored Dependencies~. It verifies the exact package version and layout, then copies the packaged files into a real, ignored Dependencies/ directory without requiring junction or symlink privileges.

Validation

Bootstrap executed twice successfully; the second run was idempotent.
