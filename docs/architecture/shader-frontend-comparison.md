...
This document states what AMUSE's two implemented shader semantic frontends —
Poiyomi Toon 9.3.64 and lilToon 2.3.4 — have taught us about the architecture.

This is a durable architecture record, not a milestone document. The record comes
from a reading of both merged implementations at `9c37f22`. A third shader frontend
should be checked against this record before anyone writes it.

**Scope note.** This document records *what generalized and what did not*. It does
not describe either shader's equations. Those equations live in the per-adapter
design specs. This document proposes no abstraction.

## Supersedes

`docs/superpowers/specs/2026-08-17-liltoon-semantics-adapter-design.md` holds an
earlier version of this comparison, written before implementation. The merged code
falsifies two of its claims. This document **supersedes** those two claims:

1. That spec states that `TryReadBinary` is deliberately duplicated because
   "lilToon's toggles are `Int`-typed with different validity ranges." The two
   implementations are **byte-identical**. lilToon reads its `Int`-typed properties
   through `GetFloat`, exactly as Poiyomi does. The duplication is real. The stated
   reason for it is not.
2. That spec presents category D as a uniformly deliberate set of shader-specific
   choices. One structural rule applied in lilToon and **not** in Poiyomi: *prove
   independent writers off before you make a neutral claim*. That divergence was
   an inconsistency, not a shader difference, and it was a confirmed correctness
   defect.
...
Both frontends implement the same five stages:
...
Both frontends separate **evidence gathering** from the **identity decision**, for
the same reason. The separation makes the identity conjunction testable without the
real shader installed. This is a confirmed pattern. Note that it is a *testability*
pattern, not a semantics pattern. Adapter #3 should follow it.

Four members are byte-identical across the two frontends and hold no shader
knowledge: `RequireAnalyzableMaterial`, `FirstFailedZeroGate`, `TryReadBinary`, and
`AllUnknown`. Four more are structurally identical but not byte-identical:
`ComputeNormalizedSourceHash` (same rule, different decomposition), `RecordUnknown<T>`,
the result/diagnostic/output types, and the value-collapse idiom. The value-collapse
idiom appears six times: four color sites compare the tint per binary32 component
(`tint.x == 1f && tint.y == 1f && tint.z == 1f ? Texture(…) : TextureTimesConstant(…)`),
plus two scalar-alpha sites compare on `colorAlpha == 1f`. Unity's aggregate `Vector3`
equality is epsilon-based, and the code deliberately avoids it: a near-one tint is a
real multiplier.

Both frontends consume **all five** `UnityTextureEvidence` facts.
...
infrastructure in the repository. It fails if a sixth non-private member appears.
It is the model for how future boundaries should be defended: a boundary nobody can
...
Poiyomi routes every sample through `_MainTex`'s sampler. lilToon does not. As a
result, one texture's import state invalidates a different set of outputs in each
frontend:
...
A new frontend is most likely to get this property wrong, because the property stays
invisible unless two texture slots are assigned at once.

## The neutral-claim gating rule

**Rule: an output may short-circuit to a neutral or zero claim only after the code
proves off every independent mechanism that could affect it.**
...
regardless of whether the *primary* slot for that output holds a value.
...
first statement, before its writer gates ran. Against the pinned upstream source, this
is a **confirmed false positive**.

`_DetailEnabled` is a `ThryToggle` bound to the `FINALPASS` shader keyword. When a
user enables it, the shader compiles in `ApplyDetailNormal`, which blends
`_DetailNormalMap` into the tangent-space normal **without reading `_BumpMap` at
all**. With `_BumpMap` unassigned, the base normal is Unity's flat `"bump"` default,
and the detail blend still perturbs it. The code refutes the remaining seven gates:
the RGB-normal writes compile out in the unlocked variant AMUSE supports, and no
decal or parallax code writes the normal.

The lesson generalizes past this one bug: **an empty slot is not evidence that the
output is unaffected.** Adapter #3 must apply the rule at every neutral-claim site.
The parity test in
`Tests/Editor/Semantics/Characterization/NeutralClaimGatingTests.cs` enforces this.

**Measured.** The parity test ran 44 cases across both frontends before any fix.
Exactly 8 failed, one per `NormalFeatureGates` entry, all at Poiyomi's Normal site.
All 36 other cases passed. The result confirms by observation that this was the one
ungated site of the eight.
...
variant. The other seven are refuted above. This distinction matters: the fix
deliberately keeps the whole gate set and moves the existing check as a unit, so
those seven gates now produce a conservative `Unknown` for cases they need not have
refused. That is an accepted false negative. It is free, and it becomes correct if
AMUSE ever supports locked materials, where the RGB-normal writes stop compiling out.

The correction is a pure reordering of the existing gate check above the
short-circuit. No gate was removed or narrowed, and the change is strictly
`Complete → Unknown` for previously unsound cases only. The full EditMode suite
passed at 629/629 afterwards, so no pre-existing test had encoded the defect.

## Repeated pressures on the semantic core

No pressure has yet justified a change to `MaterialSemantics`. Every pressure is
absorbed by a return of `Unknown`.
...
could trivially share an interface, but **nothing in the repository dispatches over
...
1. Establish a compile + EditMode CI gate. Adding a third ~1 000-line frontend to a
   repository with no automated test gate compounds risk faster than it adds
   coverage.
2. Apply the neutral-claim gating rule at every site, and extend the parity test.
3. Respect the sampler blast-radius property. Assert it explicitly for the new
   shader.
...
itself. A third toon shader would confirm little. A frontend that stresses a
different axis — a Standard/URP-lit shader, or one with no source available at all —
would test whether the four-output vocabulary is a real abstraction or a
toon-shader coincidence.

**Expect to duplicate:** its own attestation model, gate lists, equations,
diagnostic code set, and fixture shader. Copying the byte-identical helpers a third
time is correct. Extracting them *at that point* is also correct.

**Write the fixture shader to the consumed contract exactly.** Across the two
stand-in fixtures, the measurement shows: every one of lilToon's 44 fixture
properties is named in production, while 5 of Poiyomi's 94 are not (`_Mode`,
`_Cutoff`, `_SrcBlend`, `_DstBlend`, `_AlphaMask`). Neither approach is wrong. The
extra Poiyomi properties make an irrelevant-state leak testable at all, and the
lilToon fixture's exactness makes its consumed contract self-documenting. Make this
trade-off deliberately, not by accident: a fixture with no unread surface cannot
express the "irrelevant property" invariant, and must fall back to a test of
irrelevant *changes* (UV state that nothing samples through). That is what
`IrrelevantChangeInvarianceTests` does for lilToon.

## Notes toward third-party integration

This is not a current goal. This section is recorded because the two frontends make
the constraints visible.

- **Attestation is the blocker, not semantics.** Any third-party frontend must ship
  pinned digests it measured itself. No mechanism exists for that, and it would be
  premature to design one before AMUSE's own adapters are stable.
- **`UnityTextureEvidence` is the natural first public surface** — small,
  shader-independent, a refusal-predicate contract, already guarded. It is
  `internal` and should stay so until an external consumer exists.
- **The four-output vocabulary may not survive contact with a third party.** It has
  two producers, and both are toon shaders. Publishing it as an extension point
  would freeze it on the weakest evidence in this document.
